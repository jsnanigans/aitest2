...
def _create_rejected_hover(result: Dict[str, Any], data_point: Dict[str, Any]) -> str:
    ...
    if acceptance_details:
        ...
        if (
            acceptance_details.get("threshold") is not None
            and acceptance_details.get("actual_score") is not None
        ):
            lines.append(f"<b>📊 Threshold:</b> {acceptance_details['threshold']:.2f}")
            lines.append(
                f"<b>📈 Actual Score:</b> {acceptance_details['actual_score']:.2f}"
            )

        if acceptance_details.get("deviation_percentage") is not None:
            lines.append(
                f"<b>📏 Deviation:</b> {acceptance_details['deviation_percentage']:.1f}%"
            )

        if acceptance_details.get("kalman_prediction") is not None:
            lines.append(
                f"<b>🎯 Expected:</b> {acceptance_details['kalman_prediction']:.2f} kg"
            )
