...
with open(
    "/Users/brendanmullins/Projects/aitest/strem_process_anchor/output/results_test_no_date.json",
    "r",
) as f:
    data = json.load(f)
...
for user_id, measurements in data["users"].items():
    ...
    for i in range(1, len(measurements)):
        ...
        if time_hours < 1 and weight_change > 2:
            ...
            time_days < 30 and weight_change > prev["raw_weight"] * 0.5
        ):  # 50% change in a month
            problematic_changes.append(
                {
                    "user_id": user_id,
                    "time_days": time_days,
...
for issue in problematic_changes[:20]:
    if "time_minutes" in issue:
        print(
            f"User: {issue['user_id'][:8]}... | {issue['weight_change']:.1f}kg in {issue['time_minutes']:.1f} minutes"
        )
        print(f"  {issue['from']:.1f}kg -> {issue['to']:.1f}kg at {issue['timestamp']}")
    else:
        print(
            f"User: {issue['user_id'][:8]}... | {issue['weight_change']:.1f}kg ({issue['percent_change']:.0f}%) in {issue['time_days']:.1f} days"
        )
        print(f"  {issue['from']:.1f}kg -> {issue['to']:.1f}kg at {issue['timestamp']}")
