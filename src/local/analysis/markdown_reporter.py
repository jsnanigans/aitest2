...
class MarkdownReporter:
    ...
    def generate_comprehensive_report(
        ...
        report_lines.append("")
        report_lines.extend(
            self._generate_metrics_dashboard(
                raw_data, filtered_data, user_intervals, statistics
            )
        )
        report_lines.append("")
        ...
        report_lines.append("")
        report_lines.extend(
            self._generate_data_quality_analysis(
        ...
        report_lines.append("")
        report_lines.extend(
            self._generate_interval_analysis(user_intervals, statistics)
        )
        report_lines.append("")
        ...
        report_lines.append("")
        report_lines.extend(
            self._generate_weight_loss_patterns(user_intervals, statistics)
        )
        report_lines.append("")
        ...
        report_lines.append("")
        report_lines.extend(
            self._generate_outlier_analysis(
        ...
        report_lines.append("")
        report_lines.extend(self._generate_top_users_section(top_users, user_intervals))
        report_lines.append("")

        # Data Source Analysis
        report_lines.append("## 📱 Data Source Analysis")
        report_lines.append("")
        report_lines.extend(self._generate_source_analysis(raw_data, filtered_data))
        report_lines.append("")
    ...
    def _generate_executive_summary(
        self, raw_data, filtered_data, user_intervals, statistics
    ):
        ...
        rejection_rate = (
            (total_raw - total_filtered) / total_raw * 100 if total_raw > 0 else 0
        )

        lines.append("### Overview")
        lines.append("")
        lines.append(
            f"This report analyzes weight tracking data for **{total_users:,} users** over multiple 30-day intervals."
        )
        lines.append(
            f"The Kalman filtering system processed **{total_raw:,} raw measurements** and retained "
        )
        lines.append(
            f"**{total_filtered:,} measurements** after outlier detection, representing a "
        )
        ...
        successful_users = sum(
            ...
            and data["intervals"][0].get("filtered_weight")
            and data["intervals"][2].get("filtered_weight")
            and (
                data["intervals"][2]["filtered_weight"]
                < data["intervals"][0]["filtered_weight"]
            )
        )

        success_rate = successful_users / total_users * 100 if total_users > 0 else 0
        lines.append(
            f"- **Success rate** (weight loss at 90 days): {success_rate:.1f}%"
        )

        # Data quality
        users_with_complete_data = sum(
            1
            for uid, data in user_intervals.items()
            if all(i.get("filtered_weight") for i in data["intervals"][:4])
        )
        completeness = (
            users_with_complete_data / total_users * 100 if total_users > 0 else 0
        )
    ...
    def _generate_metrics_dashboard(
        ...
        total_filtered = len(filtered_data)
        lines.append(
            f"| **Total Measurements** | {total_raw:,} | {total_filtered:,} | {total_raw - total_filtered:,} ({(total_raw - total_filtered) / total_raw * 100:.1f}%) |"
        )

        # Unique users
        users_raw = raw_data["user_id"].nunique()
        users_filtered = (
            filtered_data["user_id"].nunique() if not filtered_data.empty else 0
        )
        lines.append(
            f"| **Unique Users** | {users_raw:,} | {users_filtered:,} | {users_raw - users_filtered:,} |"
        )

        # Average weight
        avg_weight_raw = raw_data["weight"].mean()
        avg_weight_filtered = (
            filtered_data["weight"].mean() if not filtered_data.empty else 0
        )
        lines.append(
            f"| **Average Weight** | {avg_weight_raw:.1f} lbs | {avg_weight_filtered:.1f} lbs | {abs(avg_weight_raw - avg_weight_filtered):.1f} lbs |"
        )
        ...
        lines.append(
            f"| **Std Deviation** | {std_raw:.1f} lbs | {std_filtered:.1f} lbs | {abs(std_raw - std_filtered):.1f} lbs |"
        )
    ...
    def _generate_data_quality_analysis(self, raw_data, filtered_data, user_intervals):
        ...
        source_counts_filtered = (
            filtered_data["source"].value_counts()
        ...
        for source in source_counts_raw.index:
            ...
            rejection_rate = (
                (raw_count - filtered_count) / raw_count * 100 if raw_count > 0 else 0
            )
            lines.append(
                f"| {source} | {raw_count:,} | {filtered_count:,} | {rejection_rate:.1f}% |"
            )
        ...
        for uid, data in user_intervals.items():
            ...
            intervals_with_data.append(
                sum(1 for i in data["intervals"] if i.get("filtered_weight"))
            )

        if measurements_per_user:
            lines.append(
                f"- **Average measurements per user**: {np.mean(measurements_per_user):.1f}"
            )
            lines.append(
                f"- **Median measurements per user**: {np.median(measurements_per_user):.0f}"
            )
            lines.append(
                f"- **Average intervals with data**: {np.mean(intervals_with_data):.1f} out of 12 possible"
            )
            lines.append(
                f"- **Users with 6+ intervals**: {sum(1 for i in intervals_with_data if i >= 6):,}"
            )
    ...
    def _generate_interval_analysis(self, user_intervals, statistics):
        ...
        for interval_stat in interval_stats_list:
            ...
            days = interval_stat.get(
                "interval_days", interval_num * 5
            )  # Now using 5-day intervals
            ...
            avg_change_raw = (
                raw_stats.get("change", {}).get("mean", 0) if raw_stats else 0
            )
            avg_change_filtered = (
                filtered_stats.get("change", {}).get("mean", 0) if filtered_stats else 0
            )
            std_dev = (
                filtered_stats.get("change", {}).get("std", 0) if filtered_stats else 0
            )
            ...
            lines.append(
                f"| {interval_num} | {days} | {count:,} | {raw_arrow} {abs(avg_change_raw):.2f} lbs | {filtered_arrow} {abs(avg_change_filtered):.2f} lbs | {std_dev:.2f} |"
            )
    ...
    def _generate_weight_loss_patterns(self, user_intervals, statistics):
        ...
        lines.append("|---------|-------|------------|-------------|")
        lines.append(
            f"| **Consistent Loss** | {len(categories['consistent_loss'])} | {len(categories['consistent_loss']) / total_categorized * 100:.1f}% | Weight loss in all intervals |"
        )
        lines.append(
            f"| **Plateau** | {len(categories['plateau'])} | {len(categories['plateau']) / total_categorized * 100:.1f}% | < 2 lbs total change |"
        )
        lines.append(
            f"| **Regain** | {len(categories['regain'])} | {len(categories['regain']) / total_categorized * 100:.1f}% | Initial loss followed by regain |"
        )
        lines.append(
            f"| **Volatile** | {len(categories['volatile'])} | {len(categories['volatile']) / total_categorized * 100:.1f}% | Mixed gains and losses |"
        )

        lines.append("")
        lines.append(
            f"*Analysis based on {total_categorized} users with 4+ intervals of data*"
        )

        return lines

    def _generate_outlier_analysis(
        self, raw_data, filtered_data, user_intervals, top_users
    ):
        ...
        lines.append(
            f"- **Total outliers removed**: {outliers_removed:,} measurements ({outliers_removed / total_raw * 100:.1f}%)"
        )
        lines.append("")
        ...
        for i, user in enumerate(top_users[:10], 1):
            ...
            lines.append(
                f"| {i} | {user_id[:8]}... | {divergence:.2f} | {avg_diff:.2f} lbs | {max_diff:.1f} lbs | {rms_diff:.2f} lbs | {num_intervals} |"
            )
    ...
    def _generate_top_users_section(self, top_users, user_intervals):
        ...
        for i, user in enumerate(top_users[:5], 1):
            ...
            lines.append("")
            lines.append(
                f"- **Signup Date**: {user_data['signup_date'].strftime('%Y-%m-%d')}"
            )
            lines.append(
                f"- **Total Measurements**: Raw={user_data['num_raw_measurements']}, Filtered={user_data['num_filtered_measurements']}"
            )
            ...
            for interval in intervals[:7]:  # Show first 6 months
                ...
                raw_w = (
                    f"{interval['raw_weight']:.1f}" if interval["raw_weight"] else "—"
                )
                filt_w = (
                    f"{interval['filtered_weight']:.1f}"
                    if interval["filtered_weight"]
                    else "—"
                )
                ...
                source = interval.get(
                    "filtered_source", interval.get("raw_source", "—")
                )
    ...
    def _generate_source_analysis(self, raw_data, filtered_data):
        ...
        for source in raw_data["source"].unique():
            ...
            filtered_source = (
                filtered_data[filtered_data["source"] == source]
                if not filtered_data.empty
                else pd.DataFrame()
            )
        ...
        for stat in source_stats:
            retention_indicator = (
                "🟢"
                if stat["retention"] > 90
                else "🟡"
                if stat["retention"] > 80
                else "🔴"
            )
            lines.append(
                f"| {stat['source']} | {stat['count']:,} | {retention_indicator} {stat['retention']:.1f}% | {stat['avg_weight']:.1f} | {stat['std_dev']:.1f} |"
            )
    ...
    def _generate_performance_section(self, phase_timings):
        ...
        for phase_key, description in phases:
            if phase_key in phase_timings:
                ...
                lines.append(
                    f"| {description} | {time_val:.2f} | {percentage:.1f}% | {indicator} |"
                )
    ...
    def _generate_recommendations(
        self, raw_data, filtered_data, user_intervals, statistics, top_users
    ):
        ...
        for source in raw_data["source"].unique():
            ...
            filtered_count = (
                len(filtered_data[filtered_data["source"] == source])
                if not filtered_data.empty
                else 0
            )
        ...
        gap_percentage = (
            users_with_gaps / len(user_intervals) * 100 if user_intervals else 0
        )

        if gap_percentage > 40:
            lines.append("### 📊 Low User Engagement")
            lines.append(
                f"- {gap_percentage:.1f}% of users have significant data gaps in first 6 months"
            )
            lines.append(
        ...
        if interval_stats_list:
            ...
            if day_90_stats:
                ...
                avg_loss = (
                    filtered_stats.get("change", {}).get("mean", 0)
                    if filtered_stats
                    else 0
                )
                if avg_loss < -5:
                    ...
                    lines.append(
                        f"- Average 90-day weight loss: {abs(avg_loss):.1f} lbs"
                    )
                    lines.append(
                    ...
                    lines.append(
                        "- **Recommendation**: Review program effectiveness and user adherence"
                    )
                lines.append("")
        ...
        if top_users and top_users[0]["divergence_score"] > 10:
            ...
            lines.append(
                f"- Some users show >10 lb average divergence between raw and filtered data"
            )
            lines.append(
