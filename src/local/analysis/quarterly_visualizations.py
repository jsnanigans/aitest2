...
class QuarterlyVisualizationGenerator:
    ...
    def create_weight_loss_distribution_comparison(
        ...
        for patch, color in zip(
            bp["boxes"], [self.colors["raw"], self.colors["filtered"]]
        ):
        ...
        ax1.set_title(
            "Weight Loss Distribution (90+ Day Users)", fontsize=13, fontweight="bold"
        )
        ...
        success_rates = pd.DataFrame(
            {
                "Raw": [
                    (results_df["raw_loss_pct"] >= 5).sum()
                    / len(results_df["raw_loss_pct"].dropna())
                    * 100,
                    (results_df["raw_loss_pct"] >= 10).sum()
                    / len(results_df["raw_loss_pct"].dropna())
                    * 100,
                    (results_df["raw_loss_pct"] >= 15).sum()
                    / len(results_df["raw_loss_pct"].dropna())
                    * 100,
                ],
                "Filtered": [
                    (results_df["filtered_loss_pct"] >= 5).sum()
                    / len(results_df["filtered_loss_pct"].dropna())
                    * 100,
                    (results_df["filtered_loss_pct"] >= 10).sum()
                    / len(results_df["filtered_loss_pct"].dropna())
                    * 100,
                    (results_df["filtered_loss_pct"] >= 15).sum()
                    / len(results_df["filtered_loss_pct"].dropna())
                    * 100,
                ],
            },
        ...
        for bar1, bar2, raw_val, filt_val in zip(
            bars1, bars2, success_rates["Raw"], success_rates["Filtered"]
        ):
            # Labels
            ax4.text(
            ...
            if filt_val > raw_val:
                ...
                ax4.annotate(
                    "",
                    xy=(bar2.get_x() + bar2.get_width() / 2, bar2.get_height() + 5),
                    xytext=(bar1.get_x() + bar1.get_width() / 2, bar1.get_height() + 5),
                    arrowprops=dict(arrowstyle="->", color="green", lw=1.5),
                )
                ax4.text(
                    (bar1.get_x() + bar2.get_x() + bar2.get_width()) / 2,
    ...
    def create_weight_loss_progression_chart(
        self, cohort_results: List[CohortAnalysis]
    ) -> str:
        """
        ...
        ax.set_title(
            "Weight Loss Progression: Raw vs Filtered Data",
            fontsize=14,
            fontweight="bold",
            pad=20,
        )
    ...
    def create_detailed_metrics_comparison(
        self, raw_metrics: QuarterlyMetrics, filtered_metrics: QuarterlyMetrics
    ) -> str:
        """
        ...
        bars1 = ax1.bar(
            x - width / 2, raw_values, width, label="Raw", color=self.colors["raw"]
        )
        bars2 = ax1.bar(
            x + width / 2,
            filtered_values,
        ...
        bars3 = ax3.bar(
            x3 - width / 2, raw_counts, width, label="Raw", color=self.colors["raw"]
        )
        bars4 = ax3.bar(
            x3 + width / 2,
            filtered_counts,
        ...
        bars5 = ax4.bar(
            x4 - width / 2, raw_pct, width, label="Raw", color=self.colors["raw"]
        )
        bars6 = ax4.bar(
            x4 + width / 2,
            filtered_pct,
        ...
        plt.suptitle(
            "Quarterly Reporting Metrics: Raw vs Filtered Data", fontsize=14, y=1.02
        )
        plt.tight_layout()
    ...
    def create_impact_summary_dashboard(
        ...
        # Main title
        fig.suptitle(
            "Quarterly Reporting Impact Dashboard: 90+ Day Users",
            fontsize=16,
            y=0.98,
            fontweight="bold",
        )
        ...
        # Create text summary
        improvement = (
            filtered_metrics.mean_weight_loss_pct - raw_metrics.mean_weight_loss_pct
        )
        improvement_text = "IMPROVED" if improvement > 0 else "DECREASED"
        color = (
            self.colors["improvement"] if improvement > 0 else self.colors["decline"]
        )

        ax1.text(
            0.5,
            0.7,
            f"Average Weight Loss {improvement_text} by",
            ha="center",
            va="center",
            fontsize=20,
            weight="bold",
        ...
        ax1.text(
            0.5,
            0.05,
            f"Raw: {raw_metrics.mean_weight_loss_pct:.2f}% → "
        ...
        success_improvements = {
            ...
            if raw_metrics.users_with_valid_endpoint > 0
            else 0,
            "10% Loss": (
                filtered_metrics.users_losing_10pct
                / filtered_metrics.users_with_valid_endpoint
        ...
        bars = ax3.bar(
            success_improvements.keys(),
            success_improvements.values(),
            color=[
                self.colors["improvement"] if v > 0 else self.colors["decline"]
        ...
        std_reduction = (
            (
                (raw_metrics.std_weight_loss_pct - filtered_metrics.std_weight_loss_pct)
                / raw_metrics.std_weight_loss_pct
                * 100
            )
            if raw_metrics.std_weight_loss_pct > 0
            else 0
        )

        ax4.text(
            0.5, 0.7, "Variability Reduced by", ha="center", va="center", fontsize=14
        )
        ax4.text(
            0.5,
            0.3,
            ...
            color=self.colors["improvement"]
            if std_reduction > 0
            else self.colors["decline"],
            weight="bold",
        )
        ax4.text(
            0.5,
            0.1,
            f"Std Dev: {raw_metrics.std_weight_loss_pct:.2f}% → "
        ...
        bars = ax5.bar(
            ...
            improvements,
            color=[
                self.colors["improvement"] if i > 0 else self.colors["decline"]
