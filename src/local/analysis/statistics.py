...
class StatisticsCalculator:
        ...
        for user_id, data in user_intervals.items():
            ...
            if baseline and final:
                ...
                raw_change = self._calculate_weight_change(
                    baseline.get("raw_weight"), final.get("raw_weight")
                )
                filtered_change = self._calculate_weight_change(
                    baseline.get("filtered_weight"), final.get("filtered_weight")
                )

                # Calculate trajectory metrics
                raw_trajectory = self._calculate_trajectory_metrics(
                    intervals, "raw_weight"
                )
                filtered_trajectory = self._calculate_trajectory_metrics(
                    intervals, "filtered_weight"
                )
                ...
                user_stats[user_id] = {
                    ...
                    "weight_change": {"raw": raw_change, "filtered": filtered_change},
                    "trajectory": {
                        "raw": raw_trajectory,
                    ...
                    "num_intervals": len(
                        [
                            i
                            for i in intervals
                            if i.get("raw_weight") or i.get("filtered_weight")
                        ]
                    ),
        ...
        for interval_num in range(max_intervals):
                ...
                if interval_num < len(user_data["intervals"]):
                    ...
                    if interval["filtered_weight"] is not None:
                        ...
                        if baseline["filtered_weight"] is not None and interval_num > 0:
                            change = (
                                interval["filtered_weight"]
                                - baseline["filtered_weight"]
                            )
            ...
            if raw_weights or filtered_weights:
                stats_dict = {
                    ...
                    "filtered": self._calculate_distribution_stats(
                        filtered_weights, filtered_changes
                    ),
                    "n_users": {
                        "raw": len(raw_weights),
                ...
                if raw_changes and filtered_changes:
                    stats_dict["difference"] = {
                        ...
                        "median_diff": np.median(filtered_changes)
                        - np.median(raw_changes),
                        "correlation": np.corrcoef(
                            raw_changes[: min(len(raw_changes), len(filtered_changes))],
                            filtered_changes[
                                : min(len(raw_changes), len(filtered_changes))
                            ],
                        )[0, 1]
                        if len(raw_changes) > 1 and len(filtered_changes) > 1
                        else None,
                    }
    ...
    def _calculate_population_statistics(self, user_intervals: Dict) -> Dict:
        ...
        return {
            ...
            "average_weight_loss": {
                ...
                "filtered": np.mean(all_filtered_changes)
                if all_filtered_changes
                else None,
            },
            "median_weight_loss": {
                "raw": np.median(all_raw_changes) if all_raw_changes else None,
                "filtered": np.median(all_filtered_changes)
                if all_filtered_changes
                else None,
            },
            "success_rate_5pct": {
                "raw": (success_rates["raw"] / total_users * 100)
                if total_users > 0
                else 0,
                "filtered": (success_rates["filtered"] / total_users * 100)
                if total_users > 0
                else 0,
            },
    ...
    def _calculate_weight_change(
        self, initial: Optional[float], final: Optional[float]
    ) -> Dict:
    ...
    def _calculate_trajectory_metrics(
        self, intervals: List[Dict], weight_key: str
    ) -> Dict:
    ...
    def _calculate_user_divergence(self, intervals: List[Dict]) -> Dict:
        ...
        return {
            ...
            "impact_score": avg_divergence
            * (
                2 - consistency
            ),  # Higher divergence and lower consistency = higher impact
        }

    def _calculate_distribution_stats(
        self, values: List[float], changes: List[float] = None
    ) -> Dict:
    ...
    def _calculate_completeness(self, intervals: List[Dict]) -> float:
        ...
        valid_count = sum(
            1 for i in intervals if i.get("raw_weight") or i.get("filtered_weight")
        )
