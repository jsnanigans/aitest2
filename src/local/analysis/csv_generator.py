...
class CSVGenerator:
    ...
    def _generate_user_progress_csv(
        self, user_intervals: Dict, data_type: str, filename: str
    ) -> Path:
                ...
                rows.append(
                    {
                        ...
                        "dataset": "raw",
                        "mean_change": raw_stats.get("change", {}).get("mean")
                        if "change" in raw_stats
                        else None,
                        "median_change": raw_stats.get("change", {}).get("median")
                        if "change" in raw_stats
                        else None,
                        "std_dev": raw_stats.get("change", {}).get("std")
                        if "change" in raw_stats
                        else None,
                        "q1": raw_stats.get("q1"),
                        "q3": raw_stats.get("q3"),
                ...
                rows.append(
                    {
                        "interval": interval["interval_days"],
                        "dataset": "filtered",
                        "mean_change": filtered_stats.get("change", {}).get("mean")
                        if "change" in filtered_stats
                        else None,
                        "median_change": filtered_stats.get("change", {}).get("median")
                        if "change" in filtered_stats
                        else None,
                        "std_dev": filtered_stats.get("change", {}).get("std")
                        if "change" in filtered_stats
                        else None,
    ...
    def _generate_user_comparison_csv(
            ...
            rows.append(
                {
                    ...
                    "weight_loss_raw": user_stats.get("weight_change", {})
                    .get("raw", {})
                    .get("percentage"),
                    "weight_loss_filtered": user_stats.get("weight_change", {})
                    .get("filtered", {})
                    .get("percentage"),
    ...
    def _add_progress_summary(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "baseline_weight" in df.columns:
            ...
            df["total_change"] = pd.to_numeric(
                df["total_change"], errors="coerce"
            ).round(2)
            df["percent_change"] = pd.to_numeric(
                df["percent_change"], errors="coerce"
            ).round(2)
    ...
    def _check_outcome_changed(self, user_stats: Dict) -> bool:
        ...
        raw_change = (
            user_stats.get("weight_change", {}).get("raw", {}).get("percentage")
        )
        filtered_change = (
            user_stats.get("weight_change", {}).get("filtered", {}).get("percentage")
        )
