...
class DailyWeightAnalyzer:
    ...
    def calculate_daily_weights(
        ...
        for user_id in all_user_ids:
            user_result = self._analyze_user_daily(
                raw_daily.get(user_id, {}), filtered_daily.get(user_id, {})
            )
    ...
    def _process_daily_data(self, df: pd.DataFrame, data_type: str) -> Dict:
        ...
        for user_id, user_data in df.groupby("user_id"):
            ...
            for date, day_data in user_data.groupby("date"):
                ...
                sources = (
                    day_data["source"].values if "source" in day_data.columns else []
                )
    ...
    def _analyze_user_daily(self, raw_daily: Dict, filtered_daily: Dict) -> Dict:
        ...
        for date in all_dates:
            ...
            if (
                raw_stats.get("mean") is not None
                and filtered_stats.get("mean") is not None
            ):
            ...
            comparison["measurements_rejected"] = raw_stats.get(
                "count", 0
            ) - filtered_stats.get("count", 0)
            ...
            if filtered_stats.get("range"):
                max_daily_range_filtered = max(
                    max_daily_range_filtered, filtered_stats["range"]
                )
        ...
        if len(all_dates) > 1:
            ...
            if baseline_filtered and filtered_daily.get(last_date, {}).get("mean"):
                filtered_total_change = (
                    filtered_daily[last_date]["mean"] - baseline_filtered
                )
            else:
        ...
        return {
            ...
            "days_with_multiple_measurements": sum(
                1 for c in daily_comparison if c["raw_count"] > 1
            ),
            ...
            "total_filtered_measurements": sum(
                c["filtered_count"] for c in daily_comparison
            ),
            "rejection_rate": 1
            - (
                sum(c["filtered_count"] for c in daily_comparison)
                / sum(c["raw_count"] for c in daily_comparison)
            )
            if sum(c["raw_count"] for c in daily_comparison) > 0
            else 0,
            "raw_total_change": raw_total_change,
            "filtered_total_change": filtered_total_change,
            "dramatic_score": dramatic_score,
            "noise_reduction": {
                "raw_avg_std": (total_raw_variance / len(all_dates)) ** 0.5
                if total_raw_variance > 0
                else 0,
                "filtered_avg_std": (total_filtered_variance / len(all_dates)) ** 0.5
                if total_filtered_variance > 0
                else 0,
            },
        }

    def _calculate_dramatic_score(
        self,
        daily_comparison: List[Dict],
    ...
    def get_most_dramatic_users(
        self, daily_results: Dict, top_n: int = 10
    ) -> List[Dict]:
        """
    ...
    def generate_daily_statistics(self, daily_results: Dict) -> Dict:
        ...
        return {
            ...
            "avg_daily_difference_across_users": float(np.mean(all_differences))
            if all_differences
            else 0,
            "median_daily_difference_across_users": float(np.median(all_differences))
            if all_differences
            else 0,
            "max_daily_difference_observed": max(
                (r["max_daily_difference"] for r in daily_results.values()), default=0
            ),
            "avg_rejection_rate": float(np.mean(all_rejection_rates))
            ...
            "overall_rejection_rate": 1
            - (total_measurements_filtered / total_measurements_raw)
            if total_measurements_raw > 0
            else 0,
        }
