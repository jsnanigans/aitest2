...
class ResetType(Enum):
    ...
    INITIAL = "initial" # First measurement - aggressive adaptation
...
class ResetManager:
            ...
            if source in MANUAL_DATA_SOURCES or source in soft_config.get('trigger_sources', []):
                ...
                if last_weight:
                    ...
                    if weight_change >= min_change:
                        # Check cooldown period (no reset in last N days)
    ...
    def get_last_reset_timestamp(state: Dict[str, Any]) -> Optional[datetime]:
        ...
        # Also check reset_timestamp field
    ...
    def get_reset_parameters(reset_type: ResetType, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns dict with:
        - weight_boost_factor: Multiplier for weight covariance
        - trend_boost_factor: Multiplier for trend covariance
        - decay_rate: Exponential decay rate
        - warmup_measurements: Number of measurements to adapt over
        - adaptive_days: Days to stay adaptive
        - quality_threshold: Quality score threshold during adaptation
        """
        ...
        defaults = {
            ...
            ResetType.SOFT: {
                ...
                'quality_consistency_weight': 0.20
            }
        }
    ...
    def perform_reset(
        ...
        new_state = {
            ...
            'measurement_history': []  # Clear history on reset
        }
    ...
    def is_in_adaptive_period(
        ...
        adaptive_days = reset_params.get('adaptive_days', 7)
        
        # In adaptive period if either condition is met
        if measurements_since < warmup_measurements or days_since < adaptive_days:
    ...
    def get_adaptive_factor(
        ...
        reset_params = state.get('reset_parameters', {})
        decay_rate = reset_params.get('decay_rate', 3)
