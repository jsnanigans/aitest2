...
def handle_process(event: Dict[str, Any], request_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        response_data = ProcessResponseData(
            user_id=user_id,
            measurements_processed=response.processed_count,
            measurements_accepted=response.accepted_count,
            measurements_rejected=response.rejected_count,
            results=[
                MeasurementResult(
                    measurement_id=m.uuid,
                    accepted=m.accepted,
                    quality_score=m.quality_score,
                    kalman_estimate=m.kalman_estimate,
                    kalman_uncertainty=m.kalman_uncertainty,
                    rejection_reason=m.rejection_reason,
                    processing_stage=m.stage,
                    reset_triggered=m.reset_triggered,
                    quality_components=m.components
                ) for m in response.measurements
            ],
            state_update=StateInfo(
                user_id=user_id,
                current_weight=response.state_update.current_weight if response.state_update else None,
                previous_weight=response.state_update.previous_weight if response.state_update else None,
                last_processed_at=response.state_update.last_processed_timestamp if response.state_update else None
            ) if response.state_update else None
        )
