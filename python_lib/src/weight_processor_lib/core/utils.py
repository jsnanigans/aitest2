    ...
    if "processing" in config:
        ...
        if "extreme_threshold" in processing:
            ...
            if not (0 < threshold < 1):
                errors.append(
                    f"Invalid extreme_threshold: {threshold} (must be between 0 and 1)"
                )
    ...
    if "quality_scoring" in config:
        ...
        if "component_weights" in qs:
            ...
            if abs(total - 1.0) > 0.001:
                errors.append(
                    f"Quality scoring weights must sum to 1.0, got {total:.3f}"
                )
    ...
    if "visualization" in config:
        ...
        if "verbosity" in viz:
            ...
            if viz["verbosity"] not in valid_verbosity:
                errors.append(
                    f"Invalid verbosity: {viz['verbosity']} (must be one of {valid_verbosity})"
                )
    ...
    if "adaptive_noise" in config:
        ...
        if "default_multiplier" in noise:
            ...
            if not (0.5 <= multiplier <= 5.0):
                errors.append(
                    f"Invalid default_multiplier: {multiplier} (should be between 0.5 and 5.0)"
                )
