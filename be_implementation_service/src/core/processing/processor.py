...
def _normalize_state_arrays(state: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure all arrays have correct 2D shape.

    Standardizes state arrays to:
    - last_state: (2, 2) - two rows of [weight, trend]
    - last_covariance: (2, 2, 2) - two 2x2 covariance matrices

    This function handles migration from old formats (1D or single-row 2D).
    """
    import numpy as np
    ...
    if state.get("last_state") is not None:
        ...
        # Convert 1D to 2D by duplicating the row
        if len(last_state.shape) == 1:
            state["last_state"] = np.array([last_state, last_state])
        # If only 1 row, duplicate it to get (2, 2)
        elif last_state.shape[0] == 1:
            state["last_state"] = np.array([last_state[0], last_state[0]])
        # If >2 rows, keep only last 2
        elif last_state.shape[0] > 2:
            state["last_state"] = last_state[-2:]

    # Same for covariance
    if state.get("last_covariance") is not None:
        last_cov = state["last_covariance"]

        # Convert 2D (2,2) to 3D (2,2,2)
        if len(last_cov.shape) == 2:
            state["last_covariance"] = np.array([last_cov, last_cov])
        # If only 1 matrix, duplicate it
        elif last_cov.shape[0] == 1:
            state["last_covariance"] = np.array([last_cov[0], last_cov[0]])
        # If >2 matrices, keep only last 2
        elif last_cov.shape[0] > 2:
            state["last_covariance"] = last_cov[-2:]

    return state


def _validate_state_shapes(state: Dict[str, Any], user_id: str) -> None:
    """Validate state arrays have correct shapes. Raises on invalid.

    Expected shapes:
    - last_state: (2, 2)
    - last_covariance: (2, 2, 2)

    Raises:
        ValueError: If shapes don't match expected values
    """
    import numpy as np

    if state.get("last_state") is not None:
        shape = state["last_state"].shape
        if shape != (2, 2):
            raise ValueError(
                f"Invalid last_state shape for {user_id}: {shape}, expected (2, 2)"
            )

    if state.get("last_covariance") is not None:
        shape = state["last_covariance"].shape
        if shape != (2, 2, 2):
            raise ValueError(
                f"Invalid last_covariance shape for {user_id}: {shape}, expected (2, 2, 2)"
            )
...
def process_measurement(
    ...
    if state is None:
        ...
        state = _normalize_state_arrays(state)
        # Validate shapes are correct
        _validate_state_shapes(state, user_id)
