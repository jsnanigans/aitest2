...
class StateValidator:
    ...
    def _validate_kalman_state(self, state: Dict) -> bool:
        ...
        for param in required_reset_params:
            ...
            if isinstance(value, (int, float)):
                ...
                if "multiplier" in param:
                    if value <= 0:
                        logger.error(
                            f"Reset parameter {param} must be positive, got {value}"
                        )
                        return False
        ...
        if state.get("measurements_since_reset", -1) != 0:
            logger.error(
                f"Measurements counter should be 0 after reset, got {state.get('measurements_since_reset')}"
            )
            return False
    ...
    def _validate_processor_state(self, state: Dict) -> bool:
        ...
        if "measurement_history" in state:
            ...
            if len(history) > max_history_size:
                logger.error(
                    f"Measurement history too large: {len(history)} > {max_history_size}"
                )
                return False
    ...
    def _validate_buffer_state(self, state: Dict) -> bool:
        ...
        if "measurement_history" in state:
            ...
            if len(history) > 100:  # Reasonable limit after reset
                logger.warning(
                    f"Large measurement history after reset: {len(history)} items"
                )
