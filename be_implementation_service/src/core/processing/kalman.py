...
# Use custom Kalman filter implementation
...
class KalmanFilterManager:
    ...
    def initialize_immediate(
        ...
        initial_row = [weight, 0]
        return {
            "kalman_params": kalman_params,
            "last_state": np.array([initial_row, initial_row]),  # (2, 2) shape
            "last_covariance": np.array([[[initial_variance, 0], [0, 0.001]], [[initial_variance, 0], [0, 0.001]]]),  # (2, 2, 2) shape
    ...
    def update_state(
        ...
        if state.get("last_state") is None:
            filtered_state_means, filtered_state_covariances = kalman.filter(
                observation
            )
            ...
            new_last_covariance = np.array(
                [last_covariance[-1], filtered_state_covariance]
            )
    ...
    def calculate_time_delta_days(
        current_timestamp: datetime, last_timestamp: Optional[datetime]
    ) -> float:
...
def get_adaptive_kalman_params(
    ...
    for key in base_config:
        if key in adaptive_params:
            ...
            result[key] = (
                adaptive_value * (1 - decay_factor) + base_value * decay_factor
            )
        else:
...
class ResetManager:
    ...
    def should_trigger_reset(
        ...
        if soft_enabled:
            if source in MANUAL_DATA_SOURCES or source in soft_config.get(
                ...
                last_weight = state.get("last_raw_weight") or state.get(
                    "last_accepted_weight"
                )
                if last_weight:
                    ...
                    if weight_change >= min_change:
                        ...
                        if (
                            not last_reset
                            or (timestamp - last_reset).total_seconds() / 86400.0
                            > cooldown_days
                        ):
    ...
    @staticmethod
    def get_reset_parameters(
        reset_type: ResetType, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        reset_config = (
            config.get("kalman", {}).get("reset", {}).get(reset_type.value, {})
        )
    ...
    def perform_reset(
        ...
        reset_event = {
            ...
            "gap_days": gap_days,
            "reason": ResetManager.get_reset_reason(
                reset_type, gap_days, weight, state
            ),
    ...
    def is_in_adaptive_period(
        state: Dict[str, Any], timestamp: datetime
    ...
    @staticmethod
    def get_adaptive_factor(state: Dict[str, Any], timestamp: datetime) -> float:
        """
