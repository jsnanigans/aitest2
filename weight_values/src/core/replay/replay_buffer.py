...
class ReplayBuffer:
    ...
    def add_measurement(
        ...
        with self._lock:
            try:
                ...
                if (
                    not buffer_data["first_timestamp"]
                    or timestamp < buffer_data["first_timestamp"]
                ):
                    buffer_data["first_timestamp"] = timestamp
                if (
                    not buffer_data["last_timestamp"]
                    or timestamp > buffer_data["last_timestamp"]
                ):
                ...
                logger.debug(
                    f"Added measurement for user {user_id}, buffer size: {len(buffer_data['measurements'])}"
                )
    ...
    def clear_buffer(self, user_id: str) -> bool:
        ...
        with self._lock:
            if user_id in self.buffers:
                ...
                logger.info(
                    f"Cleared buffer for user {user_id} (was {old_size} measurements)"
                )
                return True
            return False
    ...
    def get_ready_buffers(self) -> List[str]:
        ...
        Returns:
            List of user IDs ready for replay processing
        """
        with self._lock:
            ready_users = []
            ...
            stats.update(
                {
                    ...
                    "total_buffered_measurements": sum(
                        len(b["measurements"]) for b in self.buffers.values()
                    ),
    ...
    def _enforce_buffer_limits(self, user_id: str) -> None:
        ...
        if len(measurements) > self.max_buffer_measurements:
            ...
            if buffer_data["measurements"]:
                buffer_data["first_timestamp"] = buffer_data["measurements"][0][
                    "timestamp"
                ]

            logger.debug(
                f"Enforced buffer limit for user {user_id}, kept {len(buffer_data['measurements'])} measurements"
            )
    ...
    def cleanup(self):
        ...
        with self._lock:
            ...
            total_measurements = sum(
                len(buffer["measurements"]) for buffer in self.buffers.values()
            )
