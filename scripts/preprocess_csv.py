...
def should_filter_datetime(
    dt: Optional[datetime], min_dt: Optional[datetime], max_dt: Optional[datetime]
) -> bool:
    if dt is None:
        return True
...
def process_user_buffer(
    user_data: List[Dict], skip_duplicates: bool = True
) -> List[Dict]:
    if not user_data:
        return []
...
def write_user_data(
    writer: csv.DictWriter, user_data: List[Dict], fieldnames: List[str]
) -> int:
    rows_written = 0
...
def preprocess_csv(
    input_path: str, output_path: Optional[str] = None, config: Dict = None
) -> Dict:
        ...
        for row_num, row in enumerate(reader, 1):
            ...
            if parsed_dt is None:
                ...
                if verbose and stats["filtered_by_invalid"] <= 10:
                    print(
                        f"Warning: Could not parse date '{date_str}' at row {row_num}"
                    )
                continue
            ...
            if (
                ...
                or parsed_dt < stats["date_range"]["min"]
            ):
                stats["date_range"]["min"] = parsed_dt
            if (
                stats["date_range"]["max"] is None
                or parsed_dt > stats["date_range"]["max"]
            ):
...
def print_summary(stats: Dict, verbose: bool = True):
    ...
    if stats["source_types"]:
        ...
        for source, count in sorted(
            stats["source_types"].items(), key=lambda x: x[1], reverse=True
        )[:10]:
...
Examples:
  ...
  %(prog)s data.csv --no-verbose --keep-duplicates
    ...
    parser.add_argument(
        "-o", "--output", help="Output CSV file path (default: input_optimized.csv)"
    )
    ...
    parser.add_argument(
        "--min-date", help="Minimum date to include (YYYY-MM-DD or YYYY-MM-DD HH:MM:SS)"
    )
    parser.add_argument(
        "--max-date", help="Maximum date to include (YYYY-MM-DD or YYYY-MM-DD HH:MM:SS)"
    )
    ...
    try:
        ...
        if config["verbose"]:
            output_path = (
                args.output
                or Path(args.input_file).stem + config["output_suffix"] + ".csv"
            )
