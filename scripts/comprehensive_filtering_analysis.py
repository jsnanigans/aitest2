...
class ComprehensiveFilteringAnalyzer:
    ...
    def analyze_user_data(self, raw_df: pd.DataFrame, filtered_df: pd.DataFrame,
    ...
    def _calculate_statistical_metrics(self, raw_df: pd.DataFrame,
        ...
        metrics['std_reduction'] = (metrics['std_raw'] - metrics['std_filtered']) / metrics['std_raw'] if metrics['std_raw'] > 0 else 0
        ...
        metrics['iqr_reduction'] = (metrics['iqr_raw'] - metrics['iqr_filtered']) / metrics['iqr_raw'] if metrics['iqr_raw'] > 0 else 0

        # Coefficient of variation
        metrics['cv_raw'] = metrics['std_raw'] / metrics['mean_raw'] if metrics['mean_raw'] > 0 else 0
        metrics['cv_filtered'] = metrics['std_filtered'] / metrics['mean_filtered'] if metrics['mean_filtered'] > 0 else 0
    ...
    def _detect_outliers(self, raw_weights: np.ndarray,
                        filtered_weights: np.ndarray) -> Dict:
        ...
        metrics['removal_rate'] = metrics['points_removed'] / len(raw_weights) if len(raw_weights) > 0 else 0

        return metrics

    def _calculate_temporal_consistency(self, raw_df: pd.DataFrame,
                                       filtered_df: pd.DataFrame) -> Dict:
        ...
        if len(raw_df) > 1:
            ...
            for i in range(1, len(raw_df)):
                days_diff = (raw_df.iloc[i]['effectiveDateTime'] -
                           raw_df.iloc[i-1]['effectiveDateTime']).days
                if days_diff > 0:
                    weight_change = abs(raw_df.iloc[i]['weight'] -
                                      raw_df.iloc[i-1]['weight']) / days_diff
            ...
            if raw_changes:
                ...
                metrics['impossible_changes_raw'] = sum(c > 2.0 for c in raw_changes)  # >2kg/day

        if len(filtered_df) > 1:
            filtered_df = filtered_df.sort_values('effectiveDateTime')
            filtered_changes = []
            for i in range(1, len(filtered_df)):
                days_diff = (filtered_df.iloc[i]['effectiveDateTime'] -
                           filtered_df.iloc[i-1]['effectiveDateTime']).days
                if days_diff > 0:
                    weight_change = abs(filtered_df.iloc[i]['weight'] -
                                      filtered_df.iloc[i-1]['weight']) / days_diff
            ...
            if filtered_changes:
                ...
                metrics['impossible_changes_filtered'] = sum(c > 2.0 for c in filtered_changes)

        # Volatility index (rolling standard deviation)
        if len(raw_df) > 7:
            raw_df['weight_rolling_std'] = raw_df['weight'].rolling(window=7, min_periods=3).std()
            metrics['volatility_raw'] = raw_df['weight_rolling_std'].mean()

        if len(filtered_df) > 7:
            filtered_df['weight_rolling_std'] = filtered_df['weight'].rolling(window=7, min_periods=3).std()
        ...
        if len(raw_df) > 10 and len(filtered_df) > 10:
            ...
            raw_daily = raw_df.set_index('effectiveDateTime').resample('D')['weight'].mean()
            filtered_daily = filtered_df.set_index('effectiveDateTime').resample('D')['weight'].mean()

            # Find overlapping dates
            common_dates = raw_daily.index.intersection(filtered_daily.index)
            if len(common_dates) > 5:
                correlation = np.corrcoef(raw_daily[common_dates].values,
                                        filtered_daily[common_dates].values)[0, 1]
                metrics['trend_correlation'] = correlation

        return metrics

    def _calculate_medical_impact(self, raw_df: pd.DataFrame,
                                 filtered_df: pd.DataFrame) -> Dict:
        ...
        for days in [30, 60, 90]:
            metrics.update(self._calculate_weight_change_accuracy(
        ...
        filtered_change_pct = 100 * (initial_filtered - final_filtered) / initial_filtered

        # Check if classification changes at key thresholds
        metrics['crosses_5pct_threshold'] = (
            (raw_change_pct < 5 and filtered_change_pct >= 5) or
            (raw_change_pct >= 5 and filtered_change_pct < 5)
        )
        metrics['crosses_10pct_threshold'] = (
            (raw_change_pct < 10 and filtered_change_pct >= 10) or
            (raw_change_pct >= 10 and filtered_change_pct < 10)
        )

        # Direction agreement
        metrics['direction_agreement'] = np.sign(raw_change_pct) == np.sign(filtered_change_pct)
    ...
    def _calculate_weight_change_accuracy(self, raw_df: pd.DataFrame,
        ...
        raw_end = raw_df.iloc[(raw_df['effectiveDateTime'] - target_date).abs().argsort()[:1]]
        filtered_end = filtered_df.iloc[(filtered_df['effectiveDateTime'] - target_date).abs().argsort()[:1]]

        if not raw_end.empty and not filtered_end.empty:
            raw_change = raw_end.iloc[0]['weight'] - raw_df.iloc[0]['weight']
            filtered_change = filtered_end.iloc[0]['weight'] - filtered_df.iloc[0]['weight']
    ...
    def _analyze_source_reliability(self, raw_df: pd.DataFrame,
                                   filtered_df: pd.DataFrame) -> Dict:
        ...
        for source in source_counts.index:
            ...
            filtered_source = filtered_df[filtered_df['source'] == source] if 'source' in filtered_df.columns else pd.DataFrame()
    ...
    def generate_population_metrics(self, user_metrics: List[Dict]) -> Dict:
        ...
        population_metrics['avg_removal_rate'] = df['removal_rate'].mean() if 'removal_rate' in df else 0
        population_metrics['avg_std_reduction'] = df['std_reduction'].mean() if 'std_reduction' in df else 0

        # Medical impact summary
        if 'crosses_5pct_threshold' in df:
            population_metrics['pct_crossing_5pct'] = df['crosses_5pct_threshold'].mean() * 100
        if 'crosses_10pct_threshold' in df:
            population_metrics['pct_crossing_10pct'] = df['crosses_10pct_threshold'].mean() * 100
        if 'direction_agreement' in df:
            population_metrics['pct_direction_agreement'] = df['direction_agreement'].mean() * 100

        return population_metrics

    def analyze_quarterly_reporting_impact(self, raw_cohort_df: pd.DataFrame,
        ...
        cohort_stats_raw = self._calculate_cohort_statistics(raw_cohort_df, start_date, end_date)
        cohort_stats_filtered = self._calculate_cohort_statistics(filtered_cohort_df, start_date, end_date)

        metrics['cohort_mean_loss_raw'] = cohort_stats_raw.get('mean_weight_loss', 0)
        metrics['cohort_mean_loss_filtered'] = cohort_stats_filtered.get('mean_weight_loss', 0)
        ...
        metrics['pct_5pct_loss_filtered'] = cohort_stats_filtered.get('pct_5pct_loss', 0)
        metrics['pct_10pct_loss_raw'] = cohort_stats_raw.get('pct_10pct_loss', 0)
        metrics['pct_10pct_loss_filtered'] = cohort_stats_filtered.get('pct_10pct_loss', 0)

        # Statistical power improvement
        if metrics['cohort_std_raw'] > 0 and metrics['cohort_std_filtered'] > 0:
            # Calculate effect sizes (Cohen's d)
            effect_size_raw = abs(metrics['cohort_mean_loss_raw']) / metrics['cohort_std_raw']
            effect_size_filtered = abs(metrics['cohort_mean_loss_filtered']) / metrics['cohort_std_filtered']
    ...
    def _calculate_cohort_statistics(self, cohort_df: pd.DataFrame,
                                    start_date: datetime,
                                    end_date: datetime) -> Dict:
        ...
        if weight_changes:
            ...
            stats['pct_5pct_loss'] = sum(p >= 5 for p in pct_changes) / len(pct_changes) * 100
            stats['pct_10pct_loss'] = sum(p >= 10 for p in pct_changes) / len(pct_changes) * 100

        return stats

    def perform_statistical_tests(self, raw_data: pd.DataFrame,
        ...
        # Normality tests
        if len(raw_weights) > 3:
            stat, p_value = shapiro(raw_weights[:min(5000, len(raw_weights))])  # Shapiro has sample size limit
            tests['shapiro_raw'] = {'statistic': stat, 'p_value': p_value, 'is_normal': p_value > 0.05}

        if len(filtered_weights) > 3:
            stat, p_value = shapiro(filtered_weights[:min(5000, len(filtered_weights))])
            tests['shapiro_filtered'] = {'statistic': stat, 'p_value': p_value, 'is_normal': p_value > 0.05}

        # Variance equality test
        if len(raw_weights) > 1 and len(filtered_weights) > 1:
            stat, p_value = levene(raw_weights, filtered_weights)
            tests['levene'] = {'statistic': stat, 'p_value': p_value, 'equal_variance': p_value > 0.05}

        # Distribution comparison
        if len(raw_weights) > 1 and len(filtered_weights) > 1:
            # Kolmogorov-Smirnov test
            stat, p_value = ks_2samp(raw_weights, filtered_weights)
            tests['ks_test'] = {'statistic': stat, 'p_value': p_value, 'same_distribution': p_value > 0.05}
            ...
            if len(raw_weights) == len(filtered_weights):
                ...
                tests['wilcoxon'] = {'statistic': stat, 'p_value': p_value, 'significant_difference': p_value < 0.05}

        return tests

    def generate_comprehensive_report(self, user_metrics: List[Dict],
        ...
        report.append(f"- **Outlier Removal Rate**: {avg_removal:.1f}% of measurements identified as outliers")
        report.append(f"- **Data Quality Improvement**: {avg_std_reduction:.1f}% reduction in standard deviation")
        report.append(f"- **Medical Decision Consistency**: {direction_agreement:.1f}% agreement in weight change direction")
        ...
        for label, raw_key, filt_key in metrics_to_show:
            ...
            report.append(f"| {label} | {raw_val:.2f} | {filt_val:.2f} | {improvement:+.1f}% |")
        ...
        for method in outlier_methods:
            ...
            if key in population_metrics:
                ...
                report.append(f"- **{method.upper()} Method Detection Rate**: {rate:.2f}%")

        # Temporal Consistency
        report.append("\n### Temporal Consistency")

        temporal_metrics = [
            ('Max Daily Change', 'max_daily_change_raw_mean', 'max_daily_change_filtered_mean'),
            ('Mean Daily Change', 'mean_daily_change_raw_mean', 'mean_daily_change_filtered_mean'),
            ('Impossible Changes (>2kg/day)', 'impossible_changes_raw_mean', 'impossible_changes_filtered_mean'),
        ]
        ...
        for label, raw_key, filt_key in temporal_metrics:
            ...
            if 'Impossible' in label:
                report.append(f"| {label} | {raw_val:.0f} | {filt_val:.0f} | {raw_val - filt_val:.0f} fewer |")
            else:
                improvement = ((raw_val - filt_val) / raw_val * 100) if raw_val != 0 else 0
                report.append(f"| {label} | {raw_val:.2f} kg | {filt_val:.2f} kg | {improvement:+.1f}% |")
        ...
        for period in ['30d', '60d', '90d']:
            ...
            if diff_key in population_metrics:
                ...
                report.append(f"- **{period} Period**: Average difference of {diff:.2f} kg")
        ...
        if reporting_metrics:
            ...
            report.append("\n### User Inclusion Analysis")
            report.append(f"- **Valid Baseline (Raw)**: {reporting_metrics.get('valid_baseline_raw', 0)} users")
            report.append(f"- **Valid Baseline (Filtered)**: {reporting_metrics.get('valid_baseline_filtered', 0)} users")
            report.append(f"- **Valid Endpoint (Raw)**: {reporting_metrics.get('valid_endpoint_raw', 0)} users")
            report.append(f"- **Valid Endpoint (Filtered)**: {reporting_metrics.get('valid_endpoint_filtered', 0)} users")
            ...
            cohort_metrics = [
                ('Mean Weight Loss', 'cohort_mean_loss_raw', 'cohort_mean_loss_filtered', 'kg'),
                ('Std Deviation', 'cohort_std_raw', 'cohort_std_filtered', 'kg'),
                ('5% Loss Success', 'pct_5pct_loss_raw', 'pct_5pct_loss_filtered', '%'),
                ('10% Loss Success', 'pct_10pct_loss_raw', 'pct_10pct_loss_filtered', '%'),
            ]
            ...
            for label, raw_key, filt_key, unit in cohort_metrics:
                ...
                report.append(f"| {label} | {raw_val:.2f} {unit} | {filt_val:.2f} {unit} | {diff:+.2f} {unit} |")

            # Statistical power
            if 'effect_size_improvement' in reporting_metrics:
                improvement = reporting_metrics['effect_size_improvement']
                report.append(f"\n### Statistical Power")
                report.append(f"- **Effect Size Improvement**: {improvement:+.3f} (Cohen's d)")
        ...
        if statistical_tests:
            ...
            if 'shapiro_raw' in statistical_tests:
                ...
                report.append(f"| Shapiro-Wilk (Raw) | {test['statistic']:.4f} | {test['p_value']:.4f} | {normality} |")

            if 'shapiro_filtered' in statistical_tests:
                test = statistical_tests['shapiro_filtered']
                normality = "Normal" if test['is_normal'] else "Not Normal"
                report.append(f"| Shapiro-Wilk (Filtered) | {test['statistic']:.4f} | {test['p_value']:.4f} | {normality} |")

            if 'levene' in statistical_tests:
                test = statistical_tests['levene']
                variance = "Equal" if test['equal_variance'] else "Unequal"
                report.append(f"| Levene's Test | {test['statistic']:.4f} | {test['p_value']:.4f} | {variance} Variance |")

            if 'ks_test' in statistical_tests:
                test = statistical_tests['ks_test']
                dist = "Same" if test['same_distribution'] else "Different"
                report.append(f"| Kolmogorov-Smirnov | {test['statistic']:.4f} | {test['p_value']:.4f} | {dist} Distribution |")
        ...
        if avg_std_reduction > 20:
            report.append(f"✅ **Excellent variance reduction**: {avg_std_reduction:.1f}% exceeds 20% target")
        elif avg_std_reduction > 10:
            report.append(f"✓ **Good variance reduction**: {avg_std_reduction:.1f}%")
        else:
            report.append(f"⚠️ **Limited variance reduction**: {avg_std_reduction:.1f}% below expectations")
        ...
        success_criteria = [
            ...
            ("Outlier Detection >95%", avg_removal > 5),  # Assuming 5-10% outlier rate is good
        ...
        if avg_removal < 5:
            report.append("1. Consider adjusting outlier detection thresholds - removal rate may be too conservative")
        elif avg_removal > 20:
            report.append("1. Review outlier detection thresholds - removal rate may be too aggressive")

        if direction_agreement < 95:
            report.append("2. Investigate cases with direction disagreement for potential algorithm improvements")

        if 'effect_size_improvement' in reporting_metrics and reporting_metrics['effect_size_improvement'] < 0:
            report.append("3. Filtering may be reducing statistical power - review filtering criteria")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Comprehensive filtering effectiveness analysis')
    parser.add_argument('--raw-data', required=True, help='Path to raw data CSV')
    parser.add_argument('--filtered-data', required=True, help='Path to filtered data CSV')
    parser.add_argument('--output-dir', default='reports', help='Output directory for reports')
    parser.add_argument('--user-sample', type=int, help='Number of users to sample for analysis')
    ...
    end_date = max(raw_df['effectiveDateTime'].max(), filtered_df['effectiveDateTime'].max())
    ...
    print(f"\nKey Findings:")
    print(f"  - Outlier Removal Rate: {population_metrics.get('avg_removal_rate', 0)*100:.1f}%")
    print(f"  - Variance Reduction: {population_metrics.get('avg_std_reduction', 0)*100:.1f}%")
    print(f"  - Direction Agreement: {population_metrics.get('pct_direction_agreement', 0):.1f}%")
