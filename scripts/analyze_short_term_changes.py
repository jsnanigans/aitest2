...
def analyze_user_rejections(processor, user_id, df):
    ...
    for i, result in enumerate(results):
        if not result['accepted'] and i > 0:
            ...
            time_delta = (result['timestamp'] - prev_result['timestamp']).total_seconds() / 60
    ...
    if close_rejections:
        ...
        for min_time, max_time in time_ranges:
            range_data = df_close[(df_close['time_delta_minutes'] >= min_time) &
            ...
            if len(range_data) > 0:
                ...
                print(f"    Mean weight delta: {range_data['weight_delta'].mean():.2f}kg")
                print(f"    Reasons: {range_data['reason'].value_counts().to_dict()}")

                # Show examples
                if len(range_data) <= 3:
                    for _, row in range_data.iterrows():
                        print(f"      {row['timestamp']}: {row['weight']:.2f}kg (Δ{row['weight_delta']:.2f}kg, Q:{row['quality_score']:.2f})")
    ...
    for i, row in enumerate(user_data.itertuples()):
        if i > 0:
            ...
            time_delta = (row.effectiveDateTime - prev_row['effectiveDateTime']).total_seconds() / 60
            if time_delta < 1:
    ...
    if consecutive_groups:
        ...
        for group in consecutive_groups[:3]:  # Show first 3 groups
            ...
            print(f"    Weight range: {weight_range:.2f}kg ({min(weights):.2f} - {max(weights):.2f})")
...
def main():
    ...
    if all_close_rejections:
        ...
        recommendations = []
        for min_time, max_time in time_ranges:
            range_data = df_all[(df_all['time_delta_minutes'] >= min_time) &
        ...
        for rec in recommendations:
            if rec['samples'] > 5:  # Only if we have enough samples
                print(f"{rec['min_minutes']}-{rec['max_minutes']} min: {rec['threshold_kg']:.2f}kg")
