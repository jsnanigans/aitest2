...
def test_rapid_measurements():
    ...
    for time_diff_sec, weight_change, source, expected in test_cases:
        ...
        print(f"{status} {time_diff_sec:3d}s, {weight_change:4.2f}kg, {source:20s}: "
...
def test_user_scenario():
    ...
    measurements = [
        ...
        (base_time + timedelta(seconds=50), 103.22, "patient-device"),  # 50 seconds later, +0.07kg
    ...
    for timestamp, weight, source in measurements:
        if len(recent_weights) > 0:
            ...
            print(f"{timestamp.strftime('%H:%M:%S')}: {weight:.2f}kg ({weight_diff:+.2f}kg in {time_diff:.0f}s)")
            print(f"  Status: {status}, Score: {score:.3f}")
            if score == 0.0:
                print(f"  Rejection reason: {metadata.get('rejected_reason', 'unknown')}")
            print()
...
def test_extreme_cases():
    ...
    print(f"3kg change in 2 hours: Score={score:.3f} (should be accepted with some penalty)")
