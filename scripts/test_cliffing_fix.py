...
def analyze_cliff_user(results_file):
    ...
    if rapid_sequences:
        avg_quality = sum(r['quality_score'] for r in rapid_sequences) / len(rapid_sequences)
        print(f"Average quality score for rapid measurements: {avg_quality:.3f}")

        # Show some examples
        print("\nExample rapid measurements:")
        for r in rapid_sequences[:5]:
            print(f"  {r['timestamp']}: {r['time_diff_hours']:.2f}h apart, "
    ...
    if cliff_events:
        ...
        for c in cliff_events[:3]:
            print(f"  {c['timestamp']}: {c['weight_change_kg']:.2f}kg in {c['time_diff_hours']:.1f}h "
    ...
    print(f"  Acceptance rate: {len(march_accepted)/len(march_measurements)*100:.1f}%")

    # Calculate trajectory smoothness (variance of filtered weights)
    if march_accepted:
        filtered_weights = [m['filtered_weight'] for m in march_accepted if m.get('filtered_weight')]
    ...
    return {
        ...
        'march_acceptance_rate': len(march_accepted)/len(march_measurements)*100 if march_measurements else 0
    }
