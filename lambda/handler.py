...
from services.processor import WeightProcessorService, HistoricalConflictError
from config import load_config
from stores import get_state_db
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Routing - resource: {resource}, path: {path}, method: {http_method}"
        )

        # Route to appropriate handler (check both resource and path for compatibility)
        if (
            resource == "/api/v1/health" or path == "/api/v1/health"
        ) and http_method == "GET":
        ...
        health_info = {
            ...
            "components": {
                ...
                "processing": {
                    ...
                    "quality_scoring_enabled": os.getenv(
                        "QUALITY_SCORING_ENABLED", "true"
                    )
                    == "true",
                    "outlier_detection_enabled": os.getenv(
...
def handle_cleanup(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        response = service.cleanup(
            user_id, request.measurements, request.options.reset_state
        )
...
def handle_replay(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from services.replay import replay_measurements
