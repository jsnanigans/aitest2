#!/usr/bin/env python3
"""
Extract the minimal sequence from the 120-batch that causes divergence.
Processes measurements incrementally and stops when divergence is detected.
"""
...
def process_with_service(measurements, user_id, config):
    ...
    for i, result in enumerate(response.results):
        # Match result to measurement by index
...
def process_typescript_service(measurements, user_id, config):
    ...
    input_file = Path("/tmp/extract_divergence_ts.json")
...
def main():
    ...
    # Find index of target measurement
    target_index = None
    ...
    # Process all measurements up to and including the target
    sequence = all_measurements[:target_index + 1]
    print(f"Processing sequence of {len(sequence)} measurements...\n")
    ...
    if len(py_results) > 0:
        print(f"\nFirst Python result keys: {list(py_results[0].keys())}")
    if len(ts_results) > 0:
        print(f"First TypeScript result keys: {list(ts_results[0].keys())}")
        print(f"First TypeScript result: {ts_results[0]}")
    ...
    # Save the minimal sequence
    output_file = Path("test_fixtures/minimal_divergence_sequence.json")
    output_data = {
        "description": f"Minimal sequence from full batch that causes divergence",
    ...
    print(f"\n✅ Saved minimal sequence to: {output_file}")
    print(f"   Contains {len(sequence)} measurements up to and including the divergent one")
