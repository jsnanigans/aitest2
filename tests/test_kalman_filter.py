...
class TestKalmanFilterInitialization:
    ...
    def test_initialize_immediate_basic(
        self, default_weight, base_timestamp, empty_kalman_config
    ):
    ...
    def test_initialize_immediate_with_partial_config(self):
        ...
        weight = 65.0
        timestamp = datetime(2024, 1, 1, 10, 0)
        kalman_config = {"transition_covariance_weight": 0.5}
        ...
        assert (
            params["transition_covariance"][1][1]
            == KALMAN_DEFAULTS["transition_covariance_trend"]
        )
        assert (
            params["observation_covariance"][0][0]
...
class TestKalmanFilterUpdate:
    ...
    def test_update_state_first_measurement(self):
        ...
        updated_state = KalmanFilterManager.update_state(
            initial_state, weight, timestamp, "patient-device", {}
        )
    ...
    def test_update_state_sequence(self):
        ...
        for i, weight in enumerate(weights[1:], 1):
            ...
            state = KalmanFilterManager.update_state(
                state, weight, timestamp, "patient-device", {}
            )
            ...
            filtered_weight = (
                ...
                if len(state["last_state"].shape) > 1
                else state["last_state"][0]
            )
    ...
    def test_predict_without_observation(self):
        ...
        updated_state = KalmanFilterManager.update_state(
            state_with_trend, 70.5, next_timestamp, "patient-device", {}
        )
    ...
    def test_time_delta_calculation(self):
        ...
        state = KalmanFilterManager.update_state(
            state, 71.0, timestamp2, "patient-device", {}
        )
    ...
    def test_time_delta_capping(self):
        ...
        state = KalmanFilterManager.update_state(
            state, 75.0, timestamp2, "patient-device", {}
        )
...
class TestSourceReliability:
    ...
    def test_source_noise_multipliers(self, source, expected_multiplier):
        ...
        assert actual_multiplier == expected_multiplier, (
            f"Source {source}: expected {expected_multiplier}, got {actual_multiplier}"
        )
...
class TestEdgeCases:
    ...
    def test_extreme_weight_values(self, weight, base_timestamp, empty_kalman_config):
        ...
        assert not math.isnan(state["last_state"][0][0]), (
            f"NaN detected for weight {weight}"
        )
        assert not math.isinf(state["last_state"][0][0]), (
            f"Inf detected for weight {weight}"
        )
    ...
    def test_zero_time_delta(self):
        ...
        state = KalmanFilterManager.update_state(
            state, 71.0, timestamp, "patient-device", {}
        )
    ...
    def test_state_shape_handling(self):
        ...
        updated_state = KalmanFilterManager.update_state(
            state, 71.0, timestamp + timedelta(days=1), "patient-device", {}
        )
...
class TestResultCreation:
    ...
    def test_create_result_missing_state(self):
        ...
        state = {"last_state": None, "kalman_params": {}}
...
class TestConfidenceCalculation:
    ...
    @pytest.mark.parametrize(
        "innovation,min_conf,max_conf",
        [(0.5, 0.8, 1.0), (1.0, 0.5, 0.8), (3.0, 0.0, 0.2), (5.0, 0.0, 0.05)],
    )
    def test_calculate_confidence_ranges(self, innovation, min_conf, max_conf):
        ...
        assert min_conf < confidence < max_conf, (
            f"Confidence {confidence} not in range [{min_conf}, {max_conf}] for innovation {innovation}"
        )
...
class TestMathematicalStability:
    ...
    def test_numerical_stability_long_sequence(
        self, base_timestamp, empty_kalman_config
    ):
        ...
        for i in range(1000):
            ...
            assert 60 < filtered_weight < 80, (
                f"Weight {filtered_weight} out of bounds at iteration {i}"
            )
    ...
    def test_covariance_positive_definite(self):
        ...
        for i in range(100):
            timestamp = base_time + timedelta(days=i)
            state = KalmanFilterManager.update_state(
                state, weight + np.random.normal(0, 1), timestamp, "patient-device", {}
            )

            covariance = (
                state["last_covariance"][-1]
                if len(state["last_covariance"].shape) > 2
                else state["last_covariance"]
            )
    ...
    def test_filter_convergence(
        self, base_timestamp, empty_kalman_config, numpy_random_seed
    ):
...
class TestIntegration:
    ...
    def test_timestamp_format_handling(self):
        ...
        updated_state = KalmanFilterManager.update_state(
            state, 71.0, datetime(2024, 1, 2, 10, 0), "patient-device", {}
        )
