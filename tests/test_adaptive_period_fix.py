...
def test_adaptive_period_counter_increments_on_rejection():
    ...
    try:
        ...
        with patch('src.processing.processor.get_state_db', return_value=db):
            ...
            assert result1['accepted'] is True, f"First measurement should be accepted: {result1}"
            ...
            assert result2.get('reset_type') == 'hard' or result2.get('reset_type') == ResetType.HARD

            # Load the state to check initial counter
            state_after_reset = db.get_state(user_id)
            assert state_after_reset['measurements_since_reset'] == 1  # First accepted measurement after reset
            ...
            for result in rejected_measurements:
                assert result['accepted'] is False, "Measurement should have been rejected"
