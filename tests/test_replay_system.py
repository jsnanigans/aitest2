...
class MockDatabase:
    ...
    def get_state_snapshot_before(
        self, user_id: str, before_time: datetime
    ) -> Optional[Dict]:
    ...
    def process_measurement(
        ...
        **kwargs,
        ...
        if self.processed_count == self.fail_on_measurement:
            raise Exception(
                f"Mock processor failure at measurement {self.processed_count}"
            )
...
class TestReplayBuffer:
    ...
    def test_buffer_creation_and_basic_operations(self, buffer_config):
        ...
        # Add measurement
        user_id = "test_user"
        measurement = {"weight": 150.0, "timestamp": datetime.now(), "source": "test"}
...
class TestReplayManager:
    ...
    def test_replay_with_no_snapshot(self, mock_db, mock_processor):
        ...
        assert result["success"] == False
        assert (
            "backup" in result["error"].lower() or "snapshot" in result["error"].lower()
        )
    ...
    @patch("src.replay.replay_manager.process_measurement")
    def test_rollback_on_processor_failure(
        self, mock_process_func, mock_db, mock_processor, sample_state
    ):
    ...
    @patch("src.replay.replay_manager.process_measurement")
    def test_replay_various_sizes(
        self, mock_process_func, mock_db, mock_processor, sample_state, num_measurements
    ):
...
class TestReplayIntegration:
    ...
    @patch("src.replay.replay_manager.process_measurement")
    def test_buffer_to_replay_flow(
        self, mock_process_func, mock_db, mock_processor, buffer_config
    ):
