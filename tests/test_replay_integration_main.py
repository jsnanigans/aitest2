...
class TestReplayIntegrationMain:
    ...
    def setup_method(self):
        ...
        self.config = {
            ...
            "logging": {"progress_interval": 1000, "timestamp_format": "%Y%m%d_%H%M%S"},
        }
    ...
    def test_replay_after_reset_scenario(self):
        ...
        measurements = [
            ...
            {
                "user_id": user_id,
                "effectiveDateTime": (self.base_time + timedelta(hours=1)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            ...
            # Measurement after 20 days - should be rejected (big drop)
            {
                "user_id": user_id,
                "effectiveDateTime": (
                    self.base_time + timedelta(days=20, hours=2)
            ...
            # Measurement 1 hour later - should be accepted (closer to reset value)
            {
                "user_id": user_id,
                "effectiveDateTime": (
                    self.base_time + timedelta(days=20, hours=3)
                ).strftime("%Y-%m-%d %H:%M:%S"),
                "source_type": "patient-device",
                "weight": "98.0",
                "unit": "kg",
            ...
            # Add more measurements to trigger buffer processing
            {
                "user_id": user_id,
                "effectiveDateTime": (
                    self.base_time + timedelta(days=20, hours=4)
                ).strftime("%Y-%m-%d %H:%M:%S"),
                "source_type": "patient-device",
                "weight": "98.2",
                "unit": "kg",
        ...
        user_results, stats = stream_process(
            csv_path=csv_path, output_dir=str(self.output_dir), config=self.config
        )
        ...
        if day20_results:
            ...
            result_90kg = next(
                (r for r in day20_results if abs(r["filtered_weight"] - 90.0) < 0.1),
                None,
            )
            result_98kg = next(
                (r for r in day20_results if abs(r["filtered_weight"] - 98.0) < 0.1),
                None,
            )
            ...
            if result_90kg and result_98kg:
                ...
                print(
                    f"90kg result: accepted={result_90kg.get('accepted')}, quality={result_90kg.get('quality_score')}"
                )
                print(
                    f"98kg result: accepted={result_98kg.get('accepted')}, quality={result_98kg.get('quality_score')}"
                )
        ...
        if stats.get("replay_processed", 0) > 0:
            ...
            print(
                f"Replay stats: processed={stats['replay_processed']}, "
    ...
    def test_replay_with_normal_progression(self):
        ...
        for i in range(20):
            timestamp = self.base_time + timedelta(
                hours=i * 0.1
            )  # Quick succession to trigger buffer
        ...
        outlier_result = next(
            (r for r in results if abs(r["filtered_weight"] - 85.0) < 0.1), None
        )
        if outlier_result:
            print(
                f"Outlier (85kg) was {'accepted' if outlier_result['accepted'] else 'rejected'}"
            )
    ...
    def test_quality_score_override_in_replay(self):
        ...
        measurements = [
            ...
            {
                "user_id": user_id,
                "effectiveDateTime": (self.base_time + timedelta(minutes=10)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                "source_type": "patient-device",
                "weight": "70.2",
            ...
            {
                "user_id": user_id,
                "effectiveDateTime": (self.base_time + timedelta(minutes=20)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            ...
            {
                "user_id": user_id,
                "effectiveDateTime": (self.base_time + timedelta(minutes=30)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                "source_type": "patient-device",
                "weight": "70.3",
            ...
            {
                "user_id": user_id,
                "effectiveDateTime": (self.base_time + timedelta(hours=2)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                "source_type": "patient-device",
                "weight": "70.4",
        ...
        high_quality_result = next(
            (r for r in results if abs(r["filtered_weight"] - 75.0) < 0.1), None
        )
    ...
    @pytest.mark.parametrize(
        "trigger_mode,buffer_hours,max_measurements",
        [
            ("time_based", 1, 100),
            ("measurement_count", 72, 10),
        ],
    )
    def test_different_trigger_modes(
        self, trigger_mode, buffer_hours, max_measurements
    ):
