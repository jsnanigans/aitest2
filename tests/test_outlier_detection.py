...
def mock_db_with_kalman_state():
    ...
    db.get_state.return_value = {
        ...
        "state_history": [{"timestamp": datetime(2024, 1, 1), "state": [70.0, 0.1]}],
    }
    return db
...
class TestStatisticalMethods:
    ...
    def test_iqr_detection_normal_data(self, basic_config, normal_measurements):
        ...
        outliers = detector._detect_iqr_outliers(
            [m["weight"] for m in normal_measurements]
        )

        # Should find no outliers in normal data
        assert len(outliers) == 0

    def test_iqr_detection_with_outliers(
        self, basic_config, measurements_with_outliers
    ):
    ...
    def test_mad_detection_normal_data(self, basic_config, normal_measurements):
        ...
        detector = OutlierDetector(basic_config)
        outliers = detector._detect_zscore_outliers(
            [m["weight"] for m in normal_measurements]
        )
    ...
    def test_mad_detection_with_outliers(
        self, basic_config, measurements_with_outliers
    ):
...
class TestQualityOverrides:
    ...
    def test_high_quality_protection(
        self, basic_config, high_quality_measurements, mock_feature_manager
    ):
...
class TestKalmanDeviation:
    ...
    def test_kalman_fifteen_percent_threshold(
        self, basic_config, mock_db_with_kalman_state
    ):
        ...
        measurements = [
            {
                ...
                "timestamp": base_time,
                "metadata": {},
            },  # Exactly 15% above (70 * 1.15 = 80.5)
            {
                "weight": 59.5,
                "timestamp": base_time + timedelta(days=1),
                "metadata": {},
            },  # Exactly 15% below (70 * 0.85 = 59.5)
            {
                "weight": 80.6,
                "timestamp": base_time + timedelta(days=2),
                "metadata": {},
...
class TestIntegration:
    ...
    def test_and_logic_statistical_and_kalman(
        self, basic_config, mock_db_with_kalman_state
    ):
        ...
        measurements = [
            ...
            },
            {
                ...
                "timestamp": base_time + timedelta(days=2),
                "metadata": {},
            },  # Fails Kalman but not statistical
            {
                "weight": 85.0,
                "timestamp": base_time + timedelta(days=3),
                "metadata": {},
...
class TestAnalysisAndUtilities:
    ...
    def test_analyze_outliers(self, basic_config, measurements_with_outliers):
        ...
        analysis = detector.analyze_outliers(
            measurements_with_outliers, outlier_indices
        )
    ...
    def test_config_updates(self, basic_config):
        ...
        # Update config
        new_config = {"iqr_multiplier": 2.0, "z_score_threshold": 2.5}
