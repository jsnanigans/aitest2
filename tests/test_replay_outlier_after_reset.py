...
sys.path.insert(
    0, os_module.path.dirname(os_module.path.dirname(os_module.path.abspath(__file__)))
)
...
def test_outlier_after_reset_with_gap(setup_test):
    ...
    assert result2["reset_event"]["type"] == "hard", (
        "Should be hard reset due to 35 day gap"
    )
    ...
    print(
        f"Result 2 (120kg outlier): accepted={result2.get('accepted')}, stage={result2.get('stage')}"
    )
    ...
    if replay_triggered:
        ...
        print(
            f"Replay outliers found: {replay_metadata.get('results', {}).get('outlier_indices', [])}"
        )
    ...
    weights_in_history = [
        m.get("weight", m.get("raw_weight")) for m in measurement_history
    ]
...
def test_outlier_without_reset(setup_test):
    ...
    assert result1["accepted"], "Initial measurement should be accepted"
    assert (
        "reset_event" not in result_outlier
        or result_outlier.get("reset_event", {}).get("type") != "hard"
    ...
    print(
        f"Outlier result (120kg): accepted={result_outlier.get('accepted')}, stage={result_outlier.get('stage')}"
    )
    ...
    if not result_outlier.get("accepted"):
        print(
            f"Outlier was rejected immediately by: {result_outlier.get('rejection_reason')}"
        )
    else:
