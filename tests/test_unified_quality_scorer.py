...
class TestUnifiedQualityScorer:
    ...
    def _create_recent_weights(
        self, base_weight: float, variations: List[float], days_back: int = 20
    ) -> tuple:
    ...
    def test_post_meal_variation(self):
        ...
        assert (
            score.components["temporal_consistency"] == 0.2
        )  # Minimum score for large deviation
    ...
    def test_source_reliability_scoring(self):
        ...
        assert (
            score_reliable.components["source_reliability"]
            > score_unreliable.components["source_reliability"]
        )
