...
class TestReplayAfterReset:
    ...
    def test_snapshot_saved_after_reset(self, mock_logger, mock_get_db):
        ...
        for key, snap in self.snapshots.items():
            print(f"  {key}: reset_type={snap['state'].get('reset_type')}, measurements={snap['state'].get('measurements_since_reset')}")
        
        # CRITICAL: Verify a snapshot was saved at the reset time
        snapshot_key = f"{self.user_id}_{reset_time.isoformat()}"
        assert snapshot_key in self.snapshots, f"Snapshot should be saved immediately after reset. Available: {list(self.snapshots.keys())}"
        ...
        assert snapshot['state']['reset_type'] == 'soft', f"Expected 'soft' reset type, got '{snapshot['state']['reset_type']}'"
    ...
    def test_replay_can_restore_after_reset(self, mock_get_db):
        ...
        assert reset_result['reset_event']['type'] == 'soft', f"Expected soft reset, got {reset_result.get('reset_event')}"
        ...
        assert restore_result['success'], "Should find and restore from post-reset snapshot"
        # The snapshot should be from the soft reset time (most recent before buffer_start_time)
        assert restore_result['snapshot_timestamp'] == reset_time, f"Expected snapshot from {reset_time}, got {restore_result['snapshot_timestamp']}"
