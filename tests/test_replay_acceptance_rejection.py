...
def mock_database():
    ...
    class MockDatabase:
        ...
        def get_state_snapshot_before(self, user_id: str, before_time: datetime):
            ...
            valid_snapshots = [
                s for s in self.snapshots[user_id] if s["timestamp"] < before_time
            ]
        ...
        for rule in acceptance_rules:
            if rule["type"] == "weight_range":
                if not (rule["min"] <= weight <= rule["max"]):
                    ...
                    result["reason"] = (
                        f"Weight {weight}kg outside acceptable range [{rule['min']}, {rule['max']}]"
                    )
                ...
                quality_score = max(
                    0, 1.0 - (deviation / 50.0)
                )  # Loses 0.02 per kg deviation
                ...
                if quality_score < rule["threshold"]:
                    ...
                    result["rejected"] = True
                    result["reason"] = (
                        f"Quality score {quality_score:.2f} below threshold {rule['threshold']}"
                    )
                ...
                if abs(weight - predicted_weight) > max_deviation:
                    ...
                    result["rejected"] = True
                    result["reason"] = (
                        f"Outlier: {weight}kg deviates {abs(weight - predicted_weight):.1f}kg from predicted {predicted_weight}kg"
                    )
...
class TestPhysiologicalValidation:
    ...
    def test_weight_range_validation(
        ...
        acceptance_rules = [{"type": "weight_range", "min": 30, "max": 400}]
...
class TestQualityScoring:
    ...
    @pytest.mark.parametrize(
        "weight,expected_weight,threshold,should_accept",
        [
...
class TestOutlierDetection:
    ...
    @pytest.mark.parametrize(
        "weight,predicted,max_dev,should_accept",
        [
    ...
    def test_outlier_detection_thresholds(
        self, weight, predicted, max_dev, should_accept, test_user_id, base_timestamp
    ):
        ...
        if not should_accept:
            assert result.get("is_outlier") == True, "Should be flagged as outlier"
            assert "Outlier" in result["reason"], "Reason should mention outlier"
...
class TestSourceReliability:
    ...
    @pytest.mark.parametrize(
        "source,rejected_sources,should_accept",
        [
    ...
    def test_source_rejection(
        self, source, rejected_sources, should_accept, test_user_id, base_timestamp
    ):
        ...
        """
        acceptance_rules = [
            {"type": "source_rejection", "rejected_sources": rejected_sources}
        ]
        mock_process = create_mock_process_measurement(acceptance_rules)
...
class TestReplayAcceptanceIntegration:
    ...
    def test_mixed_acceptance_rejection(
        ...
        mock_process_func.side_effect = create_mock_process_measurement(
            acceptance_rules
        )
        ...
        measurements = [
            ...
            {
                "weight": 25.0,
                "timestamp": base_timestamp - timedelta(hours=4),
                "source": "patient-device",
            },  # ✗ Too low
            {
                "weight": 180.0,
                "timestamp": base_timestamp - timedelta(hours=3),
                "source": "patient-device",
            },  # ✗ Low quality
            {
                "weight": 165.0,
...
class TestPerformance:
    ...
    def test_large_batch_processing(
        ...
        for i in range(1000):
            ...
            measurements.append(
                {"weight": weight, "timestamp": timestamp, "source": "test"}
            )
        ...
        assert elapsed_time < 10.0, (
            f"Processing 1000 measurements should complete within 10 seconds, took {elapsed_time:.2f}s"
        )
