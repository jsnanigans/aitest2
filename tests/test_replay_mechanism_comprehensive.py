...
class TestReplayMechanism:
    ...
    def _create_mock_db(self) -> Mock:
        ...
        def save_state_snapshot(user_id, timestamp):
            ...
            if user_id in self.states:
                ...
                snapshot_id = (
                    f"{user_id}_{timestamp.isoformat()}_{self.snapshot_counter}"
                )
        ...
        def check_and_restore_snapshot(user_id, before_time):
            ...
            for key, snapshot in self.snapshots.items():
                if (
                    ...
                    and snapshot["timestamp"] < before_time
                ):
    ...
    def test_buffer_accumulates_measurements(self):
        ...
        for i in range(10):
            ...
            measurement = self._create_measurement(
                weight=70.0 + i * 0.1, timestamp=timestamp
            )
    ...
    def test_buffer_triggers_after_n_hours(self):
        ...
        final_measurement = self._create_measurement(
            weight=75.0, timestamp=final_timestamp
        )
    ...
    def test_outlier_detection_with_quality_override(self):
        ...
        detector = OutlierDetector(
            self.config["replay"]["outlier_detection"], self.mock_db
        )
        ...
        measurements = [
            ...
            self._create_measurement(
                70.2, self.base_time + timedelta(hours=1), quality_score=0.5
            ),
            self._create_measurement(
                70.1, self.base_time + timedelta(hours=2), quality_score=0.5
            ),
            self._create_measurement(
                90.0, self.base_time + timedelta(hours=3), quality_score=0.8
            ),  # Outlier with high quality
            self._create_measurement(
                70.3, self.base_time + timedelta(hours=4), quality_score=0.5
            ),
        ]
    ...
    def test_outlier_detection_statistical_methods(self):
        ...
        measurements = [
            ...
            self._create_measurement(
                70.2, self.base_time + timedelta(hours=1), quality_score=0.3
            ),
            self._create_measurement(
                70.1, self.base_time + timedelta(hours=2), quality_score=0.3
            ),
            self._create_measurement(
                95.0, self.base_time + timedelta(hours=3), quality_score=0.3
            ),  # Clear outlier
            self._create_measurement(
                70.3, self.base_time + timedelta(hours=4), quality_score=0.3
            ),
            self._create_measurement(
                45.0, self.base_time + timedelta(hours=5), quality_score=0.3
            ),  # Another outlier
            self._create_measurement(
                70.4, self.base_time + timedelta(hours=6), quality_score=0.3
            ),
        ]
    ...
    def test_replay_with_reset_scenario(self):
        ...
        measurements = [
            self._create_measurement(
                90.0, day_20_time, quality_score=0.4
            ),  # Should be rejected (big drop)
            self._create_measurement(
                98.0, hour_later, quality_score=0.6
            ),  # Should be accepted (closer to 100kg)
        ]
        ...
        detector.feature_manager.config = self.config
        clean_measurements, outlier_indices = detector.get_clean_measurements(
            measurements, self.user_id
        )
    ...
    def test_replay_with_kalman_deviation(self):
        ...
        initial_state = {
            ...
            "last_timestamp": self.base_time,
            "state_history": [
                {"timestamp": self.base_time, "state": [70.0, 0.1]},
                {"timestamp": self.base_time + timedelta(days=1), "state": [70.1, 0.1]},
                {"timestamp": self.base_time + timedelta(days=2), "state": [70.2, 0.1]},
            ],
        }
        self.mock_db.save_state(self.user_id, initial_state)
        ...
        measurements = [
            self._create_measurement(
                70.3, self.base_time + timedelta(days=3), quality_score=0.4
            ),  # Expected
            self._create_measurement(
                75.0, self.base_time + timedelta(days=4), quality_score=0.4
            ),  # Big deviation
            self._create_measurement(
                70.5, self.base_time + timedelta(days=5), quality_score=0.4
            ),  # Back to expected
        ]
    ...
    def test_full_replay_flow(self, mock_process):
        ...
        for i in range(10):
            ...
            if result["buffer_ready"]:
                ...
                # Detect outliers
                clean_measurements, outliers = detector.get_clean_measurements(
                    buffered, self.user_id
                )
