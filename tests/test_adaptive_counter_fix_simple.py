...
def test_counter_increments_during_adaptive_period():
    ...
    try:
        ...
        with patch('src.processing.processor.get_state_db', return_value=db):
            ...
            assert result2['accepted'] is True, "Post-reset measurement should be accepted"
            assert result2.get('reset_type') in ['hard', 'HARD'], "Should trigger hard reset"

            # Verify counter after accepting bad measurement
            state = db.get_state(user_id)
            assert state['measurements_since_reset'] == 1, "Counter should be 1 after first accepted measurement"
            ...
            for i in range(3):
                ...
                assert result['accepted'] is False, f"Measurement {i+1} should be rejected"
                print(f"  Measurement {i+1}: Rejected (reason: {result.get('reason', 'unknown')[:50]}...)")
                ...
                print(f"  Counter after rejection {i+1}: {actual_count} (expected: {expected_count})")
