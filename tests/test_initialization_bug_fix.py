...
def test_initialization_quality_validation():
    ...
    config = {
        ...
        "adaptive_noise": {"enabled": True, "default_multiplier": 1.5},
    }
    ...
    assert state is None or not state.get("kalman_params"), (
        "State should not be persisted for rejected initialization"
    )
    ...
    if not result["accepted"]:
        print(
            f"Result rejected: reason={result.get('reason')}, stage={result.get('stage')}"
        )
        print(
            f"Quality score: {result.get('quality_score')}, components: {result.get('quality_components')}"
        )
    ...
    assert state.get("kalman_params") is not None, (
        "Kalman state should be persisted after successful initialization"
    )
