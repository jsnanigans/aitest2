...
class TestEnhancedReplayAnalyzer:
    ...
    def test_reset_on_outlier_value(self):
        ...
        if analysis.get('scores'):
            for score in analysis['scores']:
                print(f"  Weight {score['weight']}kg: total={score['scores']['total']}, outlier={score['is_outlier']}")
        ...
        if reset_changes and reset_changes.get('should_change'):
            ...
            assert new_anchor['weight'] in [102.0, 101.0, 100.5]  # One of the reasonable values
            ...
            assert analysis.get('outliers_found', 0) > 0, "Should at least mark 150kg as outlier"
    ...
    def test_kalman_trajectory_prioritization(self):
        ...
        self.default_state['last_state'] = np.array([80.0, -0.1])  # 80kg, losing 0.1kg/day
        self.default_state['state_history'] = [
    ...
    def test_temporal_consistency_scoring(self):
        ...
        assert scores[2]['scores']['temporal'] < 0.3  # Poor temporal consistency (huge jump)
...
class TestReplayProcessor:
    ...
    def test_reset_change_handling(self):
        ...
        if result.get('analysis', {}).get('reset_changes'):
            metrics = self.processor.get_metrics()
            assert metrics['resets_changed'] >= 0  # May or may not change depending on scoring
