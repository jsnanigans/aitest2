...
def test_kalman_fit_decay_over_time():
    ...
    for days_gap, description in test_cases:
        ...
        print(f"{description}: score={score:.3f}, decay_factor={metadata.get('decay_factor', 0):.3f}")

    # Verify that scores increase with time gap
    for i in range(1, len(scores)):
        days_prev, score_prev, _ = scores[i-1]
        days_curr, score_curr, _ = scores[i]
        assert score_curr >= score_prev, f"Score should increase with time gap: {days_prev}d={score_prev:.3f} vs {days_curr}d={score_curr:.3f}"
    ...
    assert score_0d < 0.5, f"With 5kg deviation and no gap, score should be low: {score_0d:.3f}"

    # At 30 days, score should be close to 1.0 (full acceptance)
    assert score_30d > 0.95, f"At 30 days gap, score should be near 1.0: {score_30d:.3f}"
    assert metadata_30d.get('decay_factor', 0) >= 0.99, "Decay factor should be ~1.0 at 30 days"

    # Beyond 30 days should cap at same as 30 days
    assert abs(score_60d - score_30d) < 0.01, f"Score should cap at 30 days: 30d={score_30d:.3f} vs 60d={score_60d:.3f}"
...
def test_kalman_fit_no_decay_without_gap():
    ...
    assert score > 0.95, f"Perfect match with no gap should have high score: {score:.3f}"
