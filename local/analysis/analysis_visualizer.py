...
class AnalysisVisualizer:
    ...
    def generate_all_visualizations(
        self, user_intervals: Dict, statistics: Dict, top_users: List[Dict]
    ) -> Dict[str, Path]:
        """
        ...
        try:
            ...
            viz_files["weight_loss_distribution"] = (
                self._generate_weight_loss_distribution(
            ...
            viz_files["outlier_impact_heatmap"] = self._generate_outlier_impact_heatmap(
                user_intervals,
                top_users[:50],  # Top 50 for heatmap
            )
    ...
    def _generate_weight_loss_distribution(self, interval_stats: List[Dict]) -> Path:
        ...
        if raw_changes:
            ...
            axes[0].fill_between(
                raw_df["interval"], raw_df["q1"], raw_df["q3"], alpha=0.3, label="IQR"
            )
        ...
        if filtered_changes:
            ...
            axes[1].plot(
                filtered_df["interval"], filtered_df["median"], "s--", label="Median"
            )
            axes[1].fill_between(
                filtered_df["interval"],
                filtered_df["q1"],
        ...
        plt.suptitle(
            "Weight Loss Distribution: Raw vs Filtered Data",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
    ...
    def _generate_outlier_impact_heatmap(
        self, user_intervals: Dict, top_users: List[Dict]
    ) -> Path:
        ...
        max_intervals = max(len(user_intervals[uid]["intervals"]) for uid in user_ids)
        ...
        ax.set_title(
            "Outlier Impact Heatmap: Top 50 Users", fontsize=14, fontweight="bold"
        )
    ...
    def _generate_quality_correlation_plot(self, user_stats: Dict) -> Path:
        ...
        for user_id, stats in user_stats.items():
            if stats.get("data_completeness", 0) > 50:  # Only users with >50% data
                ...
                weight_change = (
                    stats.get("weight_change", {}).get("filtered", {}).get("percentage")
                )
        ...
        ax.set_title(
            "Data Quality vs Weight Loss Success", fontsize=14, fontweight="bold"
        )
        ax.legend()
...
"""
    ...
    def _generate_user_comparison_chart(
        self, user_id: str, user_data: Dict, user_stats: Dict
    ) -> Optional[Path]:
        ...
        valid_filtered = [
            (d, w) for d, w in zip(dates, filtered_weights) if w is not None
        ]
        ...
        if valid_filtered:
            ...
            ax1.plot(
                filt_dates, filt_vals, "s-", label="Filtered", alpha=0.7, linewidth=2
            )
        ...
        sources = [
            interval.get("raw_source")
            for interval in intervals
            if interval.get("raw_source")
        ]
        if sources:
        ...
        for i, interval in enumerate(intervals[1:], 1):  # Skip baseline
            if interval["interval_days"] % 30 == 0:  # Only show 30-day intervals
                ...
                if baseline_filtered and interval.get("filtered_weight"):
                    filtered_changes.append(
                        interval["filtered_weight"] - baseline_filtered
                    )
                else:
        ...
        fig.suptitle(
            f"User {user_id} - Raw Loss: {raw_pct}% | Filtered Loss: {filtered_pct}%",
            fontsize=14,
            fontweight="bold",
        )
        ...
        )  # Lower DPI for individual files
        plt.close()

        return output_path

    def _generate_top_users_dashboard(
        self, top_users: List[Dict], statistics: Dict
    ) -> Path:
        ...
        ax1.axvline(
            np.mean(divergence_scores),
            color="red",
            linestyle="--",
            label=f"Mean: {np.mean(divergence_scores):.2f}",
        )
        ax1.legend()
        ...
        for stat in statistics["interval_statistics"]:
            if stat.get("difference"):
                interval_impacts[stat["interval_days"]] = abs(
                    stat["difference"].get("mean_diff", 0)
                )
        ...
        ax4.bar(
            success_categories.keys(), success_categories.values(), color="lightgreen"
        )
        ...
        for user_info in top_users:
            user_id = user_info["user_id"]
            if user_id in statistics["user_statistics"]:
                impact = statistics["user_statistics"][user_id].get(
                    "outlier_impact", {}
                )
        ...
        if outlier_rates:
            ax5.hist(
                outlier_rates, bins=20, edgecolor="black", alpha=0.7, color="orange"
            )
        ...
        for user_info in top_users:
            user_id = user_info["user_id"]
            if user_id in statistics["user_statistics"]:
                comp = statistics["user_statistics"][user_id].get(
                    "data_completeness", 0
                )
        ...
        if completeness:
            ax6.hist(
                completeness, bins=20, edgecolor="black", alpha=0.7, color="purple"
            )
...
"""
    ...
    def _generate_filter_impact_overview(
        self, statistics: Dict, user_intervals: Dict
    ) -> Path:
        ...
        if source_stats:
            ...
            bars = ax1.bar(
                sources, rejection_rates, color=colors, edgecolor="black", alpha=0.7
            )
            ...
            ax1.axhline(
                y=10, color="gray", linestyle="--", alpha=0.5, label="10% threshold"
            )
            ax1.legend()
        ...
        total_raw = (
            sum(s.get("raw_count", 0) for s in source_stats.values())
            if source_stats
            else 0
        )
        total_filtered = (
            sum(s.get("filtered_count", 0) for s in source_stats.values())
            if source_stats
            else 0
        )
        ...
        if intervals:
            ...
            width = 0.35
            ax3.bar(
                x - width / 2, raw_stds, width, label="Raw", alpha=0.7, color="coral"
            )
            ax3.bar(
                x + width / 2,
                filtered_stds,
        ...
        for stat in interval_stats[:12]:
            if stat.get("raw") and stat.get("filtered"):
                ...
                filtered_trajectory.append(
                    stat["filtered"].get("change", {}).get("mean", 0)
                )
        ...
        if raw_stds and filtered_stds:
            ...
            if avg_raw_std > 0:
                metrics["Noise Reduction"] = (
                    (avg_raw_std - avg_filtered_std) / avg_raw_std
                ) * 100
        ...
        colors_metrics = [
            "green" if v > 70 else "orange" if v > 40 else "red" for v in metric_values
        ]
...
trend, improving data quality for analysis."""
        ...
        plt.suptitle(
            "Kalman Filter Impact Analysis", fontsize=18, fontweight="bold", y=0.98
        )
        plt.savefig(output_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def _generate_noise_reduction_demo(
        self, user_intervals: Dict, top_users: List[Dict]
    ) -> Path:
        ...
        for i, user_info in enumerate(top_users[:6]):
            ...
            valid_filtered = [
                (d, w) for d, w in zip(days, filtered_weights) if w is not None
            ]
            if valid_filtered:
                ...
                ax.plot(
                    ...
                    "b-",
                    linewidth=2.5,
                    label="Filtered (Smooth)",
                    zorder=3,
                )
                ax.scatter(
                    f_days, f_weights, c="blue", s=40, zorder=4, edgecolors="darkblue"
                )
            ...
            if len(raw_weights) > 1:
                ...
                if valid_filtered and len(valid_filtered) > 1:
                    ...
                    noise_reduction = (
                        ((raw_std - filtered_std) / raw_std * 100) if raw_std > 0 else 0
                    )
                else:
                    filtered_std = 0
                    noise_reduction = 0
            ...
            ax.set_title(
                f"User {demo_count + 1}: Noise σ={raw_std:.2f}→{filtered_std:.2f} "
                f"(-{noise_reduction:.0f}%)",
                fontweight="bold",
            )
            ax.legend(loc="best")
            ax.grid(True, alpha=0.3)
            ...
            ax.text(
                0.95,
                0.05,
                f"Divergence: {divergence:.2f} lbs",
        ...
        plt.suptitle(
            "Noise Reduction Demonstration: Raw vs Filtered Data",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
    ...
    def _generate_outlier_detection_impact(
        self, statistics: Dict, user_intervals: Dict
    ) -> Path:
        ...
        if outlier_magnitudes and retained_magnitudes:
            ax1.hist(
                [retained_magnitudes, outlier_magnitudes],
                bins=30,
                label=["Retained", "Outliers"],
                color=["green", "red"],
                alpha=0.6,
                edgecolor="black",
            )
            ...
            ax1.set_title(
                "Weight Distribution: Retained vs Outliers", fontweight="bold"
            )
            ax1.legend()
        ...
        if interval_outlier_rates:
            ax2.plot(
                interval_days, interval_outlier_rates, "o-", color="orange", linewidth=2
            )
            ax2.fill_between(
                interval_days, 0, interval_outlier_rates, alpha=0.3, color="orange"
            )
            ax2.set_xlabel("Days from Baseline")
            ...
            ax2.grid(True, alpha=0.3)
            ax2.axhline(
                y=np.mean(interval_outlier_rates),
                color="red",
                linestyle="--",
        ...
        if extreme_outliers:
            ax3.hist(
                extreme_outliers, bins=30, edgecolor="black", color="darkred", alpha=0.7
            )
            ax3.set_xlabel("Weight Adjustment (lbs)")
            ax3.set_ylabel("Frequency")
            ax3.set_title(
                f"Extreme Outliers Removed (n={len(extreme_outliers)})",
                fontweight="bold",
            )
        ...
        if source_stats:
            ...
            for i, (outlier, retained) in enumerate(
                zip(outlier_counts, retained_counts)
            ):
                ...
                if total > 0:
                    ...
                    ax4.text(
                        i,
                        outlier + retained + max(retained_counts) * 0.01,
                        f"{pct:.0f}%",
                        ha="center",
                        va="bottom",
                        fontsize=8,
        ...
        plt.suptitle(
            "Outlier Detection Impact Analysis", fontsize=16, fontweight="bold"
        )
        plt.savefig(output_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def _generate_before_after_comparison(
        ...
        for user_info in top_users:
            ...
            differences = [
                r - f if f is not None else 0
                for r, f in zip(raw_weights, filtered_weights)
            ]
            ax2.bar(
                days, differences, alpha=0.3, color="gray", width=15, label="Adjustment"
            )
            ...
            if len(raw_weights) > 1:
                ...
                if valid_filtered and len(valid_filtered) > 1:
                    ...
                    improvement = (
                        ((raw_smoothness - filt_smoothness) / raw_smoothness * 100)
                        if raw_smoothness > 0
                        else 0
                    )
                else:
            ...
            for i, (day, diff) in enumerate(zip(days, differences)):
                if abs(diff) > 5:  # Significant outlier
                    ...
                    source_short = (
                        source.split("/")[-1][:10] if "/" in source else source[:10]
                    )
                    ax.annotate(
                        source_short,
                        xy=(day, raw_weights[i]),
                        xytext=(5, 5),
        ...
        plt.suptitle(
            "Before vs After: Raw Data vs Kalman Filtered",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
    ...
    def generate_daily_comparison_visualizations(
        self, daily_results: Dict, dramatic_users: List[Dict]
    ) -> Dict[str, Path]:
    ...
    def _generate_daily_dramatic_impact(
        self, daily_results: Dict, dramatic_users: List[Dict]
    ) -> Path:
        ...
        for idx, user_info in enumerate(dramatic_users[:6]):
            ...
            filtered_means = [
                c["filtered_mean"] for c in daily_comp if c["filtered_mean"] is not None
            ]
            ...
            # Plot means
            raw_days = [
                c["days_from_start"] for c in daily_comp if c["raw_mean"] is not None
            ]
            ...
            filt_days = [
                c["days_from_start"]
                for c in daily_comp
                if c["filtered_mean"] is not None
            ]
            filt_vals = [
                c["filtered_mean"] for c in daily_comp if c["filtered_mean"] is not None
            ]
            ...
            if filt_vals:
                ax.plot(
                    filt_days, filt_vals, "b-", linewidth=2, label="Filtered Daily Avg"
                )
            ...
            ax.set_ylabel("Weight (lbs)")
            ax.set_title(
                f"User {idx + 1}: Score={user_info['dramatic_score']:.1f}, "
            ...
            ax.text(
                0.02,
                0.98,
                f"{multi_days} days with multiple readings",
                transform=ax.transAxes,
                va="top",
        ...
        plt.suptitle(
            "Daily Analysis: Most Dramatic Filter Impact\nYellow bands = days with >5 lbs difference",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
    ...
    def _generate_daily_noise_patterns(
        self, daily_results: Dict, dramatic_users: List[Dict]
    ) -> Path:
        ...
        if all_raw_ranges:
            ax1.hist(
                [all_raw_ranges, all_filtered_ranges if all_filtered_ranges else []],
                bins=30,
                label=["Raw Daily Range", "Filtered Daily Range"],
                color=["red", "blue"],
                alpha=0.6,
                edgecolor="black",
            )
            ...
            ax1.set_ylabel("Frequency")
            ax1.set_title(
                "Daily Weight Range: Raw vs Filtered (Days with Multiple Measurements)",
                fontweight="bold",
            )
            ax1.legend()
            ax1.axvline(
                np.mean(all_raw_ranges),
                color="darkred",
                linestyle="--",
        ...
        if rejection_rates and daily_variances:
            ...
            p = np.poly1d(z)
            ax2.plot(
                sorted(daily_variances), p(sorted(daily_variances)), "r--", alpha=0.8
            )
        ...
        for user_info in dramatic_users:
            user_id = user_info["user_id"]
            if user_id in daily_results:
                multi_measurement_days.append(
                    daily_results[user_id]["days_with_multiple_measurements"]
                )

        if multi_measurement_days:
            ax4.bar(
                range(len(multi_measurement_days)),
                multi_measurement_days,
                color="teal",
                edgecolor="black",
                alpha=0.7,
...
The Kalman filter effectively smooths intra-day weight fluctuations while preserving genuine weight trends."""
        ...
        plt.suptitle(
            "Daily Noise Analysis: Impact of Multiple Same-Day Measurements",
            fontsize=16,
            fontweight="bold",
        )
        plt.savefig(output_path, dpi=300, bbox_inches="tight")
