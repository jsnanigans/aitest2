...
class WeightProcessor:
    ...
    def __init__(self, user_id: str):
        ...
        self.init_buffer = []
        self.min_init_readings = 5
        
        self.min_weight = 30.0
        self.max_weight = 400.0
        self.max_daily_change = 0.03
        self.extreme_threshold = 0.30
    
    def process_weight(self, weight: float, timestamp: datetime, source: str) -> Optional[Dict[str, Any]]:
        """
        ...
        if self.last_state is not None and self.baseline:
            predicted_weight = self.last_state[0][0] + self.last_state[0][1] * time_delta_days
        ...
        if self.last_state is None:
            filtered_state_means, filtered_state_covariances = self.kalman.filter(observation)
            ...
            if self.last_state.shape[0] == 1:
                ...
                self.last_covariance = np.stack([self.last_covariance[0], filtered_state_covariance])
            else:
                self.last_state = np.array([self.last_state[-1], filtered_state_mean])
                self.last_covariance = np.array([self.last_covariance[-1], filtered_state_covariance])
        ...
        innovation_variance = self.last_covariance[-1][0, 0] + self.kalman.observation_covariance[0, 0]
        normalized_innovation = abs(innovation) / np.sqrt(innovation_variance) if innovation_variance > 0 else 0
    ...
    def _initialize_kalman(self):
        ...
        variance = np.var(weights_clean) if len(weights_clean) > 1 else 1.0
        ...
        self.kalman = KalmanFilter(
            ...
            transition_covariance=np.array([[0.5, 0], [0, 0.01]]),
            observation_covariance=np.array([[max(1.0, variance)]])
        )
