...
def create_dashboard(user_id: str, results: list, output_dir: str, viz_config: dict):
    ...
    fig.suptitle(
        f"Weight Analysis Dashboard - User {user_id[:8]}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax1.plot(
        timestamps, filtered_weights, "-", linewidth=2, label="Filtered", color="red"
    )
    ...
    ax2.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Mean: {np.mean(filtered_weights):.1f}",
    )
    ax2.axvline(
        np.median(filtered_weights),
        color="green",
        linestyle="--",
        label=f"Median: {np.median(filtered_weights):.1f}",
    )
    ...
    colors = [
        "red" if c < 0.5 else "yellow" if c < 0.8 else "green" for c in confidences
    ]
    ...
    window = viz_config.get("moving_average_window", 7)
    if len(filtered_weights) > window:
        moving_avg = np.convolve(
            filtered_weights, np.ones(window) / window, mode="valid"
        )
        ma_timestamps = (
            timestamps[window // 2 : -(window // 2)]
            if len(timestamps) > window
            else timestamps
        )
        ax5.plot(
            timestamps, filtered_weights, "o-", alpha=0.3, markersize=3, label="Daily"
        )
        ax5.plot(
            ma_timestamps,
            moving_avg,
